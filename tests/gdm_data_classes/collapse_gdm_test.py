...
def load_csvs_and_collapse(
    ...
    for csv_to_collapse in csvs_to_collapse:
        gdm_file = GDMFile.load_from_csv(csv_to_collapse, **(csv_loading_options or {}))
        # TIL OR operator can be used for False-like coalescing
        # https://docs.python.org/3/reference/expressions.html#boolean-operations

        gdm_file_filtered \
            = gdm_file.subset_based_on_callable(metadata_filter=metadata_filter) if metadata_filter else gdm_file
    ...
    for grouping_inds, grouping_gdm_file in gdm_file_all.groupby(groupby):
        if write_reports_to:
            ...
            fig_alignment, axs_alignment = plt.subplots(nrows=2, ncols=1, figsize=(12, 8), layout="constrained")
            fig_collapsing, ax_collapsing = plt.subplots(nrows=1, ncols=1, figsize=(12, 8), layout="constrained")
            report_axes = [axs_alignment[0], axs_alignment[1], ax_collapsing]
        else:
            report_axes = None

        collapsed_gdm_row = grouping_gdm_file.collapse_GDM(**(collapsing_options or {}), report_axes=report_axes)
        collapsed_gdm_file_all.append_gdm_row(collapsed_gdm_row)

        if write_reports_to:
            grouping_inds_str = '_'.join([str(x) for x in grouping_inds])
            fig_alignment.savefig(write_reports_to / f"Report_{grouping_inds_str}_Alignment.png", dpi=300)
            fig_collapsing.savefig(write_reports_to / f"Report_{grouping_inds_str}_Collapsing.png", dpi=300)
...
def get_tst_paths():
    ...
    csv_paths = [test_folder / f"{animal}.gloDatamix.csv" for animal in animals]
...
def gen_collapsing_configs():
    ...
    csv_loading_option_values = [{"metadata_only": False}, {"metadata_only": True}]
    ...
    for csv_loading_options in csv_loading_option_values:
        ...
        expected_csv = test_folder / "nanmedian_alignstarts" / "after_collapsing_expected.csv"
...
def test_compare_gdm_file_with_expected_csv_file(gdm_file: GDMFile, expected_csv: pl.Path):
    """
