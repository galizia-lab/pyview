...
class LoadMeasurementsFromFileWindow(QMainWindow):
    send_data_signal = Signal(
        MeasurementList, list, name="Measurement Selected"
    )

    def __init__(
        self, LE_loadExp, default_directory_path, measurement_list=None
    ):
    ...
    def init_UI(self):
        ...
        self.lst_file_selector.return_filename_signal.connect(
            self.load_lst_from_file
        )
        main_vbox.addWidget(self.lst_file_selector)

        self.lst_display_table = QTableWidgetPandasDF(self)
        self.lst_display_table.setSelectionBehavior(
            QAbstractItemView.SelectRows
        )
        self.lst_display_table.setSelectionMode(
    ...
    def refresh_display(self):
        self.lst_display_table.refresh(
            self.measurement_list.measurement_list_df
        )
    ...
    def row_selected(self):
        ...
        try:
            measu_header_ind = self.lst_display_table.get_headers().index(
                "Measu"
            )
        ...
        if row_selection_model.hasSelection():
            selected_measus_widget_items = [
                self.lst_display_table.item(x.row(), measu_header_ind)
                for x in row_selection_model.selectedRows()
            ]

            selected_measus = [
                int(float(x.text())) for x in selected_measus_widget_items
            ]
            ...
            QMessageBox.critical(
                self, "IO Error", "No rows selected.  Please select one!"
            )
...
class LoadMeasurementsFromVWSLogWindow(LoadMeasurementsFromFileWindow):
    ...
    def get_file_selector_combobox(self):
        lst_file_selector = get_file_selector_combobox_using_settings()(
            parent=self,
            groupbox_title="Choose a VWS.LOG File (.vws.log)",
            file_type="VWS.LOG",
            file_filter="Till VISION VWS.LOG files (*.log)",
    ...
    def load_lst_from_file(self, lst_file):
        ...
        metadata_df = importer.import_metadata(
            raw_data_files=[lst_file], measurement_filter=measurement_filter
        )

        self.measurement_list = MeasurementList.create_from_df(
            LE_loadExp=self.LE_loadExp, df=metadata_df
        )
