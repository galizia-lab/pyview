...
class LogLoadWidget(QWidget):
    ...
    def init_ui(self):
        ...
        self.choose_from_current_vws_log = QPushButton(
            "Select from current vws.log"
        )
        layout.addWidget(self.choose_from_current_vws_log)


class ListLoadWidget(QWidget):
    def __init__(self, parent, default_flags: dict):
        super().__init__(parent)
        self.new_load = None
        self.choose_from_current_list = None
        self.new_vws_log_load = None
        self.choose_from_current_vws_log = None
        self.init_ui(default_flags)

    def init_ui(self, default_flags: dict):
        layout = QVBoxLayout(self)
        ...
        self.choose_from_current_list = QPushButton(
        ...
        self.quick_load_from_current_lst_box = MainFunctionAbstract(
            parent=self.parent(),
            group_name="Quick Load",
            button_names=["Quick Load from current list"],
            flag_names=quick_load_from_current_lst_box_flags,
            flag_defaults=[
                default_flags[f] for f in quick_load_from_current_lst_box_flags
            ],
        ...
        self.selected_list_file_box = QGroupBox(
            "List/VWS.LOG file selected", self
        )
