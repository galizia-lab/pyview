...
class MainFunctionAbstract(QGroupBox):
    ...
    def __init__(
        ...
        layout = QVBoxLayout(self) if stack_vertically else QHBoxLayout(self)
        ...
        for flag, flag_default_value in zip(
            flag_names, flag_defaults, strict=True
        ):
    ...
    def collect_send_data(self):
        ...
        self.send_data.emit(
            sender.text(),
            {
                flag: flag_line_edit.text()
                for flag, flag_line_edit in self.flag_line_edits.items()
            },
        )
    ...
    def flag_update_request(self):
        ...
        self.flag_update_signal.emit(
            sender_le.accessibleName(), sender_le.text()
        )
...
class OverviewGenWidget(MainFunctionAbstract):
    ...
    def __init__(self, parent, current_flags):
        ...
        temp = {
            "CTV_FeatureNumber": "Use all features?",
            "CTV_StimulusNumber": "Use all stimuli?",
        }
    ...
    def inactivate_flag(self, state):
        ...
        sender = QObject.sender(self)
        self.deactivatable_flag_boxes[sender.accessibleName()].setEnabled(
            state != 2
        )
