...
class CentralWidget(QWidget):
    ...
    def init_ui(self):
        ...
        gen_overviews_box = OverviewGenWidget(
            parent=self, current_flags=self.flags
        )
    ...
    def enable_disable_functions_all(
        self, enable, main_exceptions=(), misc_exceptions=()
    ):

        for (
            main_function,
            main_function_widget,
        ) in self.main_function_widgets.items():
        ...
        for (
            misc_button_name,
            misc_button,
        ) in self.misc_function_buttons.items():
            if misc_button_name not in misc_exceptions:
                misc_button.setEnabled(enable)

    def enable_disable_functions(
        self, enable, main_functions=(), misc_functions=()
    ):
    ...
    def get_sample_stim_onoff_values(self):
        ...
        dict2return = [
            sample_p1.metadata.stimulus_on,
    ...
    def spawn_export_dialog(self):
        ...
        if len(self.p1s) == 0:
            QMessageBox.critical(
                self,
                "No data loaded!",
                "Please load some data before transferring data to ILTIS",
            )
            return None
        else:
            data_manager_df = self.data_manager.df
            all_metadata_columns = data_manager_df.columns.values.tolist()
            metadata_to_choose_from = set(all_metadata_columns) - set(
                self.data_manager.defaultLabelCols
                + [self.data_manager.label_col_name]
            )
            metadata_to_choose_from_list = list(metadata_to_choose_from)
            transfer_dialog = ILTISTransferDialog(
                parent=self,
                data_loaded_df=data_manager_df,
                metadata_to_choose_from=metadata_to_choose_from_list,
            )
            transfer_dialog.send_data_signal.connect(self.export_data_to_iltis)
            transfer_dialog.show()
            self.write_status(
                "Waiting for data selection before transfer to ILTIS"
            )
    ...
    def get_data_for_iltis_export(self, indices, metadata_list_for_label=None):
        ...
        for label, metadata_row in metadata_to_send.iterrows():
            ...
            stimulus_frames = (
                p1.pulsed_stimuli_handler.get_pulse_start_end_frames(
            ...
            other_metadata_to_send = metadata_row.loc[
                metadata_list_for_label
            ].values.tolist()
            label_to_use = self.data_manager.label_joiner.join(
                [le_label] + [str(x) for x in other_metadata_to_send]
            )
            if LE_loadExp == 4:
                label_to_use = (
                    f"{label_to_use}_FURA({p1.metadata.lambda_nm} nm/Ratio)"
                )
    ...
    def export_data_to_iltis_all(self):
        ...
        self.export_data_to_iltis(
            indices=slice(None), metadata_list_for_label=[]
        )
    ...
    def check_apply_gui_limitations(self, flags):
        ...
        if (
            "LE_BleachCorrMethod" in flags
            and flags["LE_BleachCorrMethod"] == "log_pixelwise"
        ):
            self.flags_widget.flag_display_choice.block_flags_update_signals(
                True
            )
            QMessageBox.warning(
                self,
                "Unavailable bleach correction setting",
    ...
    def check_update_flags_and_gui(self, flags: dict):
        ...
        for flag_name, flag_value in flags.items():
            if self.flags.is_flag_known(flag_name):
                ...
                self.write_status(
                    f"[working] Updating flag {flag_name} to {flag_value}"
                )
                try:
                    ...
                    QMessageBox.critical(
                        self,
                        f"Error setting flag {flag_name} to {flag_value}",
                        str(ase),
                    )
                    self.write_status(
                        f"[failure] Updating flag {flag_name} to {flag_value}"
                    )
                    self.flags_widget.flag_display_choice.block_flags_update_signals(
                        False
                    )
                    ...
                    QMessageBox.critical(
                        self,
                        f"Error setting flag {flag_name}",
                ...
                self.write_status(
                    f"[success] Updating flag {flag_name} to {flag_value}"
                )
            else:
                self.write_status(
                    f"[info] Ignoring update request for unknown flag {flag_name}"
                )
        return 1
    ...
    def load_yml_flags(self, yml_filename):
        ...
        self.check_update_flags_and_gui(
        ...
        if self.check_update_flags_and_gui(yml_flags):
            ...
            self.enable_disable_functions(
                ...
                main_functions=[
                    "load_lst",
                    "select row from new vws log file",
                ],
            )

            self.write_status(
                f"[success] Initializing flags from {yml_filename}"
            )

        else:
            self.write_status(
                f"[failure] Initializing flags from {yml_filename}"
            )
        ...
        self.enable_disable_functions(
            enable=False,
            main_functions=(
                "select row from current list",
                "quick load from current list",
    ...
    def go_to_wiki(self):
        QDesktopServices.openUrl(
            QUrl("https://github.com/galizia-lab/pyview/wiki")
        )

    @Slot(name="write yml file")
    def write_yml_file(self):

        filename, filters = qtpy.compat.getsavefilename(
            caption="Select a YML file for saving flags",
        ...
        try:
            ...
            QMessageBox.critical(
                self, f"VIEW encountered a {type(e).__name__}", msg
            )
    ...
    def direct_load_finalize(
        ...
        try:
            for label, p1 in label_p1_mapping.items():
                revised_label = self.data_manager.add_data(
                    flags_used, p1, label
                )
    ...
    def load_from_new_list(self):
        ...
        self.load_measurement_window.send_data_signal.connect(
            self.load_lst_data
        )
    ...
    def load_from_new_vws_log(self):
        ...
        self.load_measurement_window = LoadMeasurementsFromVWSLogWindow(
            self.flags["LE_loadExp"], default_directory_path=data_path
        )
        self.load_measurement_window.send_data_signal.connect(
            self.load_lst_data
        )
    ...
    def load_lst_data(self, measurement_list, selected_measus):
        ...
        lst_or_log_filepath = pl.Path(
            measurement_list.last_measurement_list_fle
        )
        ...
        temp_dir = (
            pl.Path(lst_or_log_filepath).parent / "temp_dir_for_view_analyses"
        )
        flags_to_update = {}
        if lst_or_log_filepath.suffixes == [".vws", ".log"]:
            ...
            flags_to_update["STG_Datapath"] = pl.Path(
                lst_or_log_filepath
            ).parent
            flags_to_update["STG_OdorInfoPath"] = str(temp_dir.parent)
        else:
            flags_to_update = {
                "STG_OdorInfoPath": str(
                    measurement_list.get_STG_OdorInfoPath()
                )
            }
        ...
        for measu in selected_measus:
            self.write_status(
                f"[working] Loading meta data for measu={measu} from {lst_or_log_filepath}."
            )

            try:
                p1_metadata, extra_metadata = (
                    measurement_list.get_p1_metadata_by_measu(measu)
                )
            except AssertionError as ase:
                QMessageBox.critical(
            ...
            self.write_status(
                f"[success] Loading meta data for measu={measu} from {lst_or_log_filepath}."
            )

            if not self.check_update_flags_and_gui(
                {
                    "STG_Measu": measu,
            ...
            try:
                ...
                self.write_status(
                    f"[failure] Loading measurement data for measu={measu} from {lst_or_log_filepath}."
                )
                return
    ...
    def choose_row_from_current_list(self):
        ...
        try:
            ...
            self.load_measurement_window.lst_file_selector.set_entry(
    ...
    def generate_overview(
        self, button_name, flags, use_all_features, use_all_stimuli
    ):
    ...
    def save_movie_new(self):
        ...
        op_name_without_extension = (
            op_path / f"{selected_data_label}(manually created)"
        )

        try:
            self.write_status(
                f"[working] Save movie with new method to {str(op_name_without_extension)}"
            )
            op_name_with_extension = export_movie(
                p1=selected_p1,
                flags=self.flags,
            ...
            QMessageBox.information(
                ...
                f"Output file: {op_name_with_extension}",
            )
    ...
    def show_foto1(self):
        ...
        try:
            foto1_data = get_foto1_data(
                flags=self.flags, p1=self.get_selected_data_p1()
            )
    ...
    def viz_gdm_traces(self):
        ...
        try:
            self.gdm_viz_window = GDMViz(
                p1=self.get_selected_data_p1(), flags=self.flags
            )
    ...
    def button3_func(self):
        ...
        QMessageBox.information(
            self,
            "Not implemented yet!",
            f"To implement this function, add code "
