...
class GDMViz(QMainWindow):
    ...
    def __init__(self, p1, flags):
        ...
        self.roi_label_trace_dict = get_roi_gdm_traces_dict(
            p1=p1, flags=flags, roi_data_dict=roi_data_dict
        )
        ...
        self.overview_canvas.figure.suptitle(
            ...
            "Each ROI has a different color,\n and selected ROIs a slightly brighter",
            size=4,
        )
        ...
        self.overview_canvas.axes.imshow(
            np.flip(overview_for_output, axis=0), origin="lower"
        )
        self.overview_canvas.axes.tick_params(labelsize=4)

        # fake points for label
        for label, (_, color) in self.roi_mask_dict.items():
            self.overview_canvas.axes.plot(
                [0], [0], color=color, ls="-", label=label
            )
        ...
        for i in range(3):
            ...
            roi_chooser.addItems(
                list(roi_data_dict.keys())
                + ["---Choose a ROI to visualize---"]
            )
    ...
    def refresh(self):
        ...
        for chosen_label in chosen_labels:
            ...
            self.trace_canvas.axes.plot(
                trace, ls="-", marker=None, color=chosen_roi_color
            )

        for this_stimulus_frames in self.stimuli_frames:
            self.trace_canvas.axes.axvspan(
                *this_stimulus_frames, color="gray", alpha=0.5, edgecolor=None
            )
        ...
        self.trace_canvas.axes.tick_params(
            labelsize=4, grid_color="k", grid_linestyle="--", grid_alpha=0.1
        )
        x_end = self.trace_canvas.axes.get_xlim()[1]
        self.trace_canvas.axes.set_xticks(
            np.linspace(0, x_end, 10).astype(int)
        )
