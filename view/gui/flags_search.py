...
from view.python_core.appdirs import get_app_usr_data_dir
...
def check_if_index_is_correct(index_dir: pl.Path):
    ...
    if not index_dir.is_dir():
        return False
    return bool(windex.exists_in(str(index_dir), indexname=get_indexname()))
...
def get_schema():
    ...
    return wfields.Schema(
        flag_name=wfields.KEYWORD(stored=True),
        flag_subgroup=wfields.STORED,
...
def create_new_index(index_dir, flags):
    ...
    ix = windex.create_in(
        dirname=str(index_dir), schema=get_schema(), indexname=get_indexname()
    )
...
def query(index, query_str: str, max_results: int = 5):
    ...
    with index.searcher() as searcher:
        ...
        description_qp = wqparser.QueryParser(
            "flag_description", schema=index.schema
        )
        ...
        for hit in name_results:
            if len(highlights) < max_results:
                ...
                highlight["flag_description"] = (
                    f"{hit['flag_description'][:100]}..."
                )
        ...
        for hit in desc_results:
            if len(highlights) < max_results:
                highlight = {}
                highlight["flag_description"] = (
                    f"...{hit.highlights('flag_description')}..."
                )
                highlight["flag_subgroup"] = hit["flag_subgroup"]
