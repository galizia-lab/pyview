...
class OldFileHandlerBlank:
    ...
    def write_old_values(
        self,
        df,
        columns,
        measu_col_name: str = "Measu",
...
class OldFileHandler(OldFileHandlerBlank):
    ...
    def backup(self):
        ...
        backup_path = lst_fle_path.with_suffix(
            f".{datetime.datetime.now().strftime('%Y%m%d%H%M%S')}{lst_fle_path.suffix}"
        )
        ...
        logging.getLogger("VIEW").info(
            "Backing up file", extra={"backup_path": backup_path}
        )
...
class OldFileHandlerMeasurementList(OldFileHandler):
    ...
    def write_old_values(
        ...
        columns = set(columns).intersection(
            set(self.old_lst_df.columns.values)
        )

        mask = self.old_lst_df[measu_col_name].apply(
            lambda x: x in df[measu_col_name].values
        )
        indices2use = mask.index.values[mask.values].tolist()

        old_df_subset = self.old_lst_df.reindex(
            index=indices2use, columns=columns
        ).set_index(measu_col_name)

        df_temp = df.set_index(measu_col_name)

        combined_df = df_temp.combine(
            old_df_subset, func=lambda s1, s2: s2, overwrite=False
        )
