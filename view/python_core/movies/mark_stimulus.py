...
class NoStimulus:
    ...
    def get_frame_odor_conc(self, frame_time):
        ...
        [odors], [concs] = self.pulsed_stimuli_handler.get_odor_info_at_times(
            [frame_time]
        )
...
class SquareStimulus(NoStimulus):
    def __init__(
        self, mv_xgap, mv_ygap, fg_color_for_pil, pulsed_stimuli_handler
    ):
...
class BaseTextStimulus(NoStimulus):
    def __init__(
        ...
        stimulus_strings = [
            self.compose_stim_text(odor, conc)
            for odor, conc in zip(all_odors, all_concs, strict=True)
        ]
        stimulus_string_lengths = [len(x) for x in stimulus_strings]
        longest_stimulus_string = stimulus_strings[
            stimulus_string_lengths.index(max(stimulus_string_lengths))
        ]

        self.font_size = get_maximum_font_size_by_width(
            maximum_width=frame_size[0],
            text=longest_stimulus_string,
            font_name=font_file,
        )
...
def get_stimulus_marker(
    ...
    if mv_markStimulus == 0:
        ...
        return SquareStimulus(
            mv_xgap=left_xgap,
            mv_ygap=mv_ygap,
            fg_color_for_pil=color_for_pil,
            pulsed_stimuli_handler=pulsed_stimuli_handler,
        )
