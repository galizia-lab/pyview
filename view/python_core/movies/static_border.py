...
from view.python_core.utils.fonts import resolve_font_file, resolve_font_size
from view.python_core.utils.pil_helpers import (
    add_string,
    numpy_to_pil_image,
    pil_image_to_numpy,
)
...
class StaticBorder:
    def __init__(
        ...
        possible_frame_Xsize = (
            self.frame_size[0] + mv_xgap_left + mv_xgap_right
        )
        ...
        self.frame_with_blank_border = np.full(
            buffer_shape, self.bg_color_for_mpl
        )
    ...
    def composite(self, frame_data, static_frame):
        ...
        assert (
            data_shape == self.frame_size
        ), f"Frame data of shape {self.frame_size} expected, got {data_shape}"

        new_frame = static_frame.astype(dtype=frame_data.dtype, copy=True)

        new_frame[
            self.mv_xgap_left : self.mv_xgap_left + data_shape[0],
            self.mv_ygap : self.mv_ygap + data_shape[1],
        ] = frame_data
...
class StaticBorderWithColorbar(StaticBorder):
    def __init__(
        ...
        super().__init__(
            mv_xgap_left, mv_xgap_right, mv_ygap, bg_color_for_mpl, frame_size
        )
        ...
        self.font_size = resolve_font_size(
            text="-0.000",
            maximum_width=mv_xgap_right,
            font_name=font_file,
        ...
        self.colorbar_x_start = current_frame_size[0] - int(
            0.65 * mv_xgap_right
        )
        ...
        self.colorbar_y_end = int(
            min(
                frame_size[1] + mv_ygap,
                current_frame_size[1] - 1 * self.font_size,
            )
        )
    ...
    def get_static_frame(self, data_to_01_mapper):
        ...
        # draw scalebar
        data_for_colorbar = np.linspace(
            vmin, vmax, self.colorbar_y_end - self.colorbar_y_start + 1
        )
        data_for_colorbar_scaled = data_to_01_mapper.normalize(
            data_for_colorbar
        )
        self.frame_with_blank_border[
            self.colorbar_x_start : self.colorbar_x_end + 1,
        ...
        upper_lower_text_x = (
            0.8 * self.colorbar_x_start + 0.2 * self.colorbar_x_end
        )
        ...
        pil_image = add_string(
            ...
            position=(
                upper_lower_text_x,
                pil_image.height
                - (self.colorbar_y_end + 0.25 * self.font_size),
            ),
            font_size=self.font_size,
            text=upper_limit_to_print_str,
            horizontal_alignment="center",
            vertical_alignment="bottom",
            fill_color_for_pil=self.fg_color_for_pil,
        ...
        pil_image = add_string(
            ...
            position=(
                upper_lower_text_x,
                pil_image.height
                - (self.colorbar_y_start - 0.25 * self.font_size),
            ),
            font_size=self.font_size,
            text=lower_limit_to_print_str,
            horizontal_alignment="center",
        ...
        if vmin <= 0 <= vmax:
            ...
            zero_y = (
                self.colorbar_y_start
                + (normalized_0 - 0) / 1 * colorbar_y_range
            )

            pil_image = add_string(
                pil_image,
                position=(
                    1.025 * self.colorbar_x_end,
                    pil_image.height - zero_y,
                ),
                font_size=self.font_size,
                text="0",
            ...
            img_draw_obj.line(
                xy=(
                    (self.colorbar_x_start, pil_image.height - zero_y),
                    (self.colorbar_x_end, pil_image.height - zero_y),
                ),
                fill="rgb(0, 0, 0)",
                width=int(np.ceil(colorbar_y_range / 128)),
            )
...
def get_static_border_adder_3D(
    flags,
    fg_color_for_mpl,
    bg_color_for_mpl,
    colormap,
    ...
    if flags["CTV_scalebar"]:
        check_frame_height(
            frame_height=frame_size[1], SO_cutborder=flags["mv_cutborder"]
        )

        mv_xgap_right = 2 * mv_xgap
        static_border_adder = StaticBorderWithColorbar(
            mv_xgap_left=mv_xgap,
            mv_xgap_right=mv_xgap_right,
            mv_ygap=mv_ygap,
...
def get_static_border_adder_2D(
    flags, fg_color_for_mpl, bg_color_for_mpl, colormap, frame_size
):

    if (
        flags["CTV_scalebar"]
        and (flags["SO_xgap"] > 0)
        and (np.floor(flags["SO_showROIs"] / 10) != 2)
    ):
        check_frame_height(
            frame_height=frame_size[1], SO_cutborder=flags["SO_cutborder"]
        )
