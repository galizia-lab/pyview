from view.python_core.movies.frame_time import get_time_string_adder
from view.python_core.movies.mark_stimulus import get_stimulus_marker
from view.python_core.movies.static_border import get_static_border_adder_3D
from view.python_core.utils.colors import mpl_color_to_PIL
from view.python_core.utils.fonts import (
    get_maximum_font_size_by_width,
    resolve_font_file,
)
...
class BordersAndAnnotations:
    def __init__(
        ...
        time_font_size_suggestion_full = get_maximum_font_size_by_width(
            maximum_width=frame_size[0],
            font_name=font_file,
            text="00 m 00.000 s",
        )
        if flags["mv_displayTime"] > 0:
            time_font_size_suggestion = max(
                int(flags["mv_displayTime"] * time_font_size_suggestion_full),
                8,
            )
        else:
        ...
        if self.stimulus_marker.font_size is not None:
            font_size_suggestion = max(
                min(time_font_size_suggestion, self.stimulus_marker.font_size),
                8,
            )
        else:
        ...
        self.static_border_adder, revised_right_xgap = (
            get_static_border_adder_3D(
                flags=flags,
    ...
    def add(self, frame_data, frame_number, static_frame):
        ...
        frame_data_with_border = self.static_border_adder.composite(
        ...
        pil_image_with_time_stimulus = self.stimulus_marker.mark(
            pil_image_with_time, frame_time
        )
