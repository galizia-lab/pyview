...
class Mask:
    ...
    def get_mask_2D(self, data):
        ...
        return (
            self.area_masker.get_area_mask_2D()
            & self.threshold_masker.get_thresh_mask_2D(data)
        )

    def get_mask_3D(self, data):

        return (
            self.area_masker.get_area_mask_3D()
            & self.threshold_masker.get_thresh_mask_3D(data)
        )
...
class ThresholdMaskStatic(ThresholdMaskBlank):
    def __init__(self, threshold_on, threshold_pos, threshold_neg, data_size):
        ...
        self.thresh_mask = apply_threshold(
            threshold_on, threshold_pos, threshold_neg
        )
...
class ThresholdMaskDynamic(ThresholdMaskBlank):
    ...
    def get_thresh_mask_2D(self, data):
        ...
        raise AttributeError(
            "ThresholdMaskDynamic object cannot generate a 2D mask, it should only be used "
...
def get_thresholder_3D(flags, p1, area_mask_excluded, excluder):
    ...
    if flags["mv_withinArea"]:
        area_masker = AreaMask(
            movie_size=revised_movie_size, frame_mask=area_mask_excluded
        )
    else:
    ...
    if flags["mv_thresholdOn"] in ("none", "None", "NONE"):
        ...
        threshold_masker = ThresholdMaskStatic(
            data_size=revised_movie_size,
            threshold_on=foto1_data,
            threshold_pos=mv_thresholdPos,
            threshold_neg=mv_thresholdNeg,
        )
    elif flags["mv_thresholdOn"] == "raw1":
        threshold_masker = ThresholdMaskDynamic(
            data_size=revised_movie_size,
            threshold_on=p1.raw1,
            threshold_pos=mv_thresholdPos,
            threshold_neg=mv_thresholdNeg,
        )
...
def get_thresholder_2D(flags, p1, area_mask_excluded, excluder):
    ...
    revised_movie_size = excluder.revise_frame_size(
    ...
    if flags["SO_thresholdOn"] in ("none", "None", "NONE"):
        ...
        threshold_masker = ThresholdMaskStatic(
            data_size=revised_movie_size,
            threshold_on=foto1_data,
            threshold_pos=so_thresholdPos,
            threshold_neg=so_thresholdNeg,
        )
    elif flags["SO_thresholdOn"] == "overview":
        threshold_masker = ThresholdMaskRunTime(
            data_size=revised_movie_size,
            threshold_pos=so_thresholdPos,
            threshold_neg=so_thresholdNeg,
        )
        ...
        raise NotImplementedError(
            f"The value of the flag 'SO_thresholdOn' was set to {flags['SO_thresholdOn']}"
