...
from view.python_core.gdm_generation.gdm_data_classes import GDMFile, GDMRow
...
def apply_mask_to_data(roi_data_dict, data_xyt, area=None):
    ...
    for label, roi_data in roi_data_dict.items():
        if area is not None:
            weighted_mask_xy = roi_data.get_weighted_mask_considering_area(
                area
            )
        else:
        ...
        roi_label_traces_dict[label] = (data_txy * weighted_mask_xy).sum(
            axis=(1, 2)
        )
...
def get_roi_gdm_traces_dict(p1, flags, roi_data_dict):
    ...
    if flags["GDM_withinArea"]:
        return apply_mask_to_data(
            data_xyt=p1.sig1, roi_data_dict=roi_data_dict, area=p1.area_mask
        )
    else:
        return apply_mask_to_data(
            data_xyt=p1.sig1, roi_data_dict=roi_data_dict
        )
...
def get_glodatamix_row_boiler_plate(p1, animal_name=None):
    ...
    temp = {
        k: p1.metadata[v]
        for k, v in list_column_p1_metadata_mapping.items()
...
def create_gdm_file_basic(
    common_metadata: pd.Series,
    roi_label_gdm_traces_dict: dict,
    sampling_period_ms: int,
    trace_onset: float = 0,
    roi_descriptions: dict = None,
    roi_label_additional_metadata: dict = None,
):
    gdm_file = GDMFile()
    ...
    for roi_label, gdm_trace in roi_label_gdm_traces_dict.items():
        ...
        if roi_label_additional_metadata is not None:
            ...
            for k, v in roi_label_additional_metadata.get(
                roi_label, {}
            ).items():
                metadata_boiler_plate[k] = v

        gdm_row = GDMRow.from_data_and_metadata(
            metadata_dict=metadata_boiler_plate,
            trace=gdm_trace,
            sampling_period_ms=sampling_period_ms,
            starting_time_s=trace_onset,
        )
...
class FullTraceGDMGenerator:
    def __init__(self, p1, flags, additional_metadata=None):
        ...
        """
        self.roi_data_dict, self.roi_file = get_roi_io_class(
            flags["RM_ROITrace"]
        ).read(flags=flags, measurement_label=p1.metadata.ex_name)
        ...
        self.roi_label_gdm_traces_dict = get_roi_gdm_traces_dict(
            p1=p1, flags=flags, roi_data_dict=self.roi_data_dict
        )
    ...
    def get_gdm_file(self):
        ...
        for roi_label, trace in self.roi_label_gdm_traces_dict.items():
            roi_label_additional_metadata[roi_label] = {
                self.ctv_name: self.calc_ctv(trace)
            }
...
class ChunksOnlyGDMGenerator(FullTraceGDMGenerator):
    ...
    def get_gdm_file(self, write_ctv=True):
        ...
        for _ind, (
            ...
            end_td,
            sampling_period_td,
        ) in self.pulsed_stimuli_handler.stimulus_frame.iterrows():
            start_sec = start_td.total_seconds()
            end_sec = end_td.total_seconds()

            chunk_start_td = start_td - pd.to_timedelta(
                self.gdm_chunkPreStim, "s"
            )
            chunk_end_td = end_td + pd.to_timedelta(
                self.gdm_chunkPostStim, "s"
            )

            chunk_slice_start_ind = np.round(
                chunk_start_td / sampling_period_td
            ).astype(int)
            chunk_slice_end_ind = np.round(
                chunk_end_td / sampling_period_td
            ).astype(int)

            chunk_slice_start_ind = max(0, chunk_slice_start_ind)

            chunk_start_quantized_ms = (
                chunk_slice_start_ind * self.sampling_period_ms
            )
            ...
            common_metadata["StimONms"] = (
                start_sec * 1000 - chunk_start_quantized_ms
            )  # in ms
            ...
            for roi_label, gdm_trace in self.roi_label_gdm_traces_dict.items():
                chunk_slice_end_ind = min(
                    gdm_trace.shape[0], chunk_slice_end_ind
                )

                chunk = gdm_trace[
                    chunk_slice_start_ind : chunk_slice_end_ind + 1
                ]
                roi_label_chunk_dict[roi_label] = chunk

                roi_label_additional_metadata[roi_label] = {
                    self.ctv_name: self.calc_ctv(chunk)
                }
