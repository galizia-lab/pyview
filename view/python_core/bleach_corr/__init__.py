...
from view.python_core.bleach_corr.pixelwise import bleach_correct_pixelwise


class NoBleachCompensator:
    def __init__(self):
...
class UniformBleachCompensator(BaseBleachCompensator):
    ...
    def apply(self, stack_xyt: np.ndarray, area_mask: np.ndarray):
        ...
        F0_frame = stack_xyt[
            :, :, self.background_frames[0] : self.background_frames[1] + 1
        ].mean(axis=2)
        ...
        corrected_F_by_F0_xyt = (
            F_by_F0_xyt - fitted_curve + fitted_curve.mean()
        )

        # converting to txy as it easier to multiply by a frame in this format
        corrected_F_by_F0_txy = np.moveaxis(
            corrected_F_by_F0_xyt, source=-1, destination=0
        )

        corrected_raw_txy = corrected_F_by_F0_txy * F0_frame

        # converting back to our stadard format xyt
        raw_corrected = np.moveaxis(
            corrected_raw_txy, source=0, destination=-1
        )
...
def get_bleach_compensator(flags, p1_metadata, movie_size):
    ...
    if flags["LE_BleachCorrMethod"] == "log_pixelwise_parallel":
        return PixelWiseBleachCompensatorParallel(
            flags, p1_metadata, movie_size
        )
        ...
        logging.getLogger("VIEW").warning(
            "Not a valid bleach value",
            extra={"LE_BleachCorrMethod": flags["LE_BleachCorrMethod"]},
        )
