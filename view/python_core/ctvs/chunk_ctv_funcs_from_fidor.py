...
def ctv_for_FIDOR_chunks_with_minmax_indices(curve, stim_frame=None):
    ...
    base_ind, peak_ind = ctv_for_FIDOR_chunks_frames(
        curve, stim_frame=stim_frame
    )
    output = curve[peak_ind] - curve[base_ind]
...
def ctv_for_FIDOR_chunks_frames(curve, stim_frame=None):
    ...
    """
    middle = stim_frame if stim_frame is not None else len(curve) // 2
    ...
    base_ind = np.argmin(
        curve[:stim_range]
    )  # minimum in first quarter (or first half before stim-time)
    base_val = curve[base_ind]
    peak_ind_max = (
        np.argmax(curve[stim_range : (middle + stim_range)]) + stim_range
    )
    peak_ind_min = (
        np.argmin(curve[stim_range : (middle + stim_range)]) + stim_range
    )
