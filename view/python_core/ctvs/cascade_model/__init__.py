...
from view.python_core.ctvs.cascade_model.gekko_related import (
    GekkoSolver,
    ModelOneComp,
    ModelTwoComp,
    ModelTwoCompNoDelay,
...
def estimate_delay_rising_phase_only(
    stimulus_trace: Sequence[float], output_trace: Sequence[float]
):
    """
    ...
    stimulus_trace_non_rising_mask = (
        np.diff(stimulus_trace) <= 0
    ).tolist() + [False]
    stim_min, stim_max = min(stimulus_trace), max(stimulus_trace)
    stimulus_trace_copy = np.array(stimulus_trace, copy=True)
    stimulus_trace_copy[stimulus_trace_non_rising_mask] = np.median(
        stimulus_trace
    )
    stimulus_trace_copy[
        stimulus_trace_copy >= stim_min + 0.85 * (stim_max - stim_min)
    ] = np.median(stimulus_trace)

    output_trace_non_rising_mask = (np.diff(output_trace) <= 0).tolist() + [
        False
    ]
    ...
    output_trace_copy[
        output_trace_copy >= output_min + 0.85 * (output_max - output_min)
    ] = np.median(output_trace)
...
def predict_into_future(fit_params, factor):
    ...
    prediction_end = (
        time_trace[max_pos] + 6 * longest_time_constant
    )  # units is seconds

    predicted_trace = None
    if prediction_end > prediction_start:
        prediction_time_trace = np.arange(
            prediction_start, prediction_end + sampling_period, sampling_period
        )
        ...
        predicted_traces_dict = gs.solve(
            ...
            state_variable_init_dict={
                k: fit_params[k][-2:] for k in gs.state_variables
            },
    ...
    if (predicted_trace is not None) and (
        predicted_trace[-1] <= predicted_trace[-2]
    ):
...
def fit_cascade_model(
    ...
    if delays_to_test is None:
        delays_to_test = np.arange(-15, 9, 1).astype(int)
    ...
    assert (
        stimulus_trace.shape == output_trace.shape
    ), "Stimulus and output traces have different shapes"
    assert (
        time_trace.shape == output_trace.shape
    ), "time trace and output trace have different shapes"
    ...
    for delay in delays_to_test:
        if delay > 0:
            ...
            output_trace_to_fit = output_trace[-delay:]
            stimulus_trace_to_fit = stimulus_trace[
                : stimulus_trace.shape[0] + delay
            ]
        ...
        if fit_params is not None:
            ...
            fit_params["output_trace_expected"] = (
                factor * fit_params["output_trace_expected"]
            )
    ...
    if len(fit_params_all) == 0:
        ...
        param_fixed_arg = {
            k: [fit_params_best_one_comp[k]] * 3 for k in params_fixed
        }
        param_fixed_arg["kF"] = [
        ...
        param_inits = {
            ...
            for k in fit_params_best_one_comp["model_params"]
            if k not in param_fixed_arg
        }

        model_two_comp_no_delay = ModelTwoCompNoDelay(
            param_inits=param_inits, params_fixed=param_fixed_arg
        )

        models_to_fit.append(model_two_comp_no_delay)

        second_comp_delays = list(range(20))
        ...
        for model2fit, _second_comp_delay in zip(
            models_to_fit, second_comp_delays, strict=True
        ):
            fit_params = fit_compare_models(
                time_vec=fit_params_best_one_comp["time_trace_fitted"],
                input_vec=fit_params_best_one_comp["stimulus_trace_fitted"],
                output_vec=factor
                * fit_params_best_one_comp["output_trace_expected"],
        ...
        if len(fit_params_all):
            ...
            fit_params_best_two_comp["delay_second_comp"] = second_comp_delays[
                best_ind
            ]
            cascade_model_output["fit_params_two_comp"] = (
                fit_params_best_two_comp
            )

            # https://imaging.mrc-cbu.cam.ac.uk/statswiki/FAQ/AICreg
            # difference of 50 chosen based fitting on a test set
            if (
                fit_params_all_second[best_ind]["bic"]
                >= fit_params_best_one_comp["bic"] - 50
            ):
                best_ind = None
        ...
        predicted_trace, predicted_time_trace = predict_into_future(
            fit_params_best, factor
        )

        cascade_model_output.update(
            {
                "sign_factor": factor,
                "fit_params": fit_params_best,
                "predicted_time_trace": predicted_time_trace,
