...
class SpatialFootprintROIData(BaseROIData):
    ...
    def __init__(
        ...
        thresh=75,
        basic_text_description="A spatial footprint ROI",
    ):
        """
        ...
        self.spatial_footprint_norm = spatial_footprint / np.nansum(
            spatial_footprint
        )

        self.value_at_thresh_percentile = np.nanpercentile(
            self.spatial_footprint_norm, self.thresh
        )
    ...
    def get_perimeter_mask(
        self, frame_size: typing.Iterable[int] = None
    ) -> typing.Tuple[np.ndarray, np.ndarray]:

        if frame_size is not None:
            assert (
                frame_size == self.spatial_footprint_norm.shape
            ), f"This SpatialFootprintROI has a shape of {self.spatial_footprint_norm.shape}, while the requested perimeter mask shape is {frame_size}"
        else:
            frame_size = self.spatial_footprint_norm.shape

        perimeter_mask = np.zeros(frame_size, dtype=bool)

        for contour_coords in find_contours(
            self.spatial_footprint_norm, self.value_at_thresh_percentile
        ):
            contour_coords_int = contour_coords.astype(int)
            perimeter_mask[
                contour_coords_int[:, 0], contour_coords_int[:, 1]
            ] = True
    ...
    def get_weighted_mask(
        ...
        if frame_size is not None:
            assert (
                frame_size == self.spatial_footprint_norm.shape
            ), f"This SpatialFootprintROI has a shape of {self.spatial_footprint_norm.shape}, while the requested mask shape is {frame_size}"
    ...
    def get_boolean_mask_without_perimeter(
        ...
        if frame_size is None:
            return super().get_boolean_mask_without_perimeter(
                frame_size=self.spatial_footprint_norm.shape
            )
        else:
