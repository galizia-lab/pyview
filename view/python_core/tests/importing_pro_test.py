...
from view.idl_folder_translation.pro2tapestry_conf import (
...
def test_parsing_pro():
    ...
    expected_csv = (
        pl.Path(get_internal_test_files_path())
        / "pro_tests"
        / "grl_A_5803a_expected.csv"
    )

    pro_data_temp_op = (
        pl.Path(tempfile.gettempdir()) / "grl_A_5803_temp_out.csv"
    )
...
def test_converting_pro():
    ...
    convert_pro_to_tapestry_config(
        output_yml_file=test_op_yml,
        input_pro_file=test_file,
        animal_tag="A_5803a",
        flags_to_override={},
    )
