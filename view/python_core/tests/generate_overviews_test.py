...
class OverviewsGenerator:
    ...
    def generate(self, flags_to_update, suffix):
        ...
        test_folder = (
            pl.Path(self.view.flags["STG_OdorReportPath"])
            / "test_single_overviews"
        )
        ...
        op_file_name = (
            test_folder
            / f"{self.view.flags['STG_ReportTag']}_{self.view.p1.metadata.ex_name}{suffix}.tif"
        )
...
def test_no_colorbar():
    ...
    generate_overviews(
        flags_to_update={"CTV_scalebar": 0}, suffix="_no_scalebar"
    )
...
def test_scale_flags():
    ...
    flags_with_percentile = {
        ...
        "SO_percentileValue": percentile_value,
    }
    ...
    for label, flags_to_copy in flag_types.items():
        ...
        for indiscale in [3, 13, 23]:
            ...
            generate_overviews(
                flags, f"SO_individualScale{indiscale}_factor0p25{label}"
            )
            ...
            generate_overviews(
                flags, f"SO_individualScale{indiscale}_factor0p4{label}"
            )
            ...
            generate_overviews(
                flags, f"SO_individualScale{indiscale}_factor0{label}"
            )
...
def test_thresholdShowImage():
    ...
    for threshold_show_image in ["foto1", "bgColor"]:
        for threshold_scale in ["full", "onlyShown"]:
            generate_overviews(
                {
                    "SO_thresholdShowImage": threshold_show_image,
...
def test_bgColor_fgColor():
    ...
    generate_overviews(
        {"SO_bgColor": "y", "SO_fgColor": "r"}, "_bgColor_yellow_fg_color_red"
    )
...
def test_showROIs():
    ...
    for test_value in test_values:
        ...
        og.generate(
            flags_to_update={"SO_showROIs": test_value},
            suffix=f"_showROI{test_value}",
        )

    og = OverviewsGenerator()
    og.generate(
        {"SO_showROIs": 15, "SO_cutborder": 5}, "showROIs15_cutborder5"
    )
