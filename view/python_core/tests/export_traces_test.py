...
class TraceExporter(object):
    def __init__(self):
        ...
        super().__init__()
        test_yml, self.test_animal, self.test_measu = (
    ...
    def load_and_export(self, flags_to_update, file_suffix, flags_suffix):
        ...
        for measu in self.view.get_measus_for_current_animal(
            analyze_values_to_use=(1,)
        ):
            ...
            gdm_file_this_measu, _ = (
                self.view.get_gdm_file_for_current_measurement(roi_data_dict)
            )
        ...
        test_output_file = (
            test_gdm_folder / f"gdm{flags_suffix}{output_file_path.suffix}"
        )
