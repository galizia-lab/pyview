...
from view.python_core.gdm_generation.gdm_data_classes import (
    read_chunks_gdm_csv,
)
from view.python_core.tests.gdm_data_classes.common import get_tst_path
...
def cases_configs():
    ...
    for ind, parameter_set in enumerate(parameter_sets):
        read_csv_df = read_chunks_gdm_csv(
            input_csv=test_path / "simple_small1.gdm.csv", **parameter_set
        )

        expected_csv = (
            test_path / "read_chunks_gdm_csv" / expected_csv_filenames[ind]
        )
...
def test_read_csv_assert_equality(
    csv_df: pd.DataFrame, expected_csv: pl.Path | str
):
