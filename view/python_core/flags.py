...
from view.python_core.paths import (
    check_get_file_existence_in_folder,
...
def get_internal_flags_def():
    ...
    flags_def_XL = importlib.resources.files("view").joinpath(
    ...
    with importlib.resources.as_file(flags_def_XL) as fle:
        flags_def_df = pd.read_csv(
            fle, comment="#", keep_default_na=False, na_values=[""]
        )

    # initialize descriptions with empty string if it has no entry
    flags_def_df["Flag Description"] = flags_def_df["Flag Description"].apply(
...
def check_correct_flag(
    flag_value,
    flag_name: str,
    expected_value_type_str: str,
    flag_checks: str = "True",
    ...
    if comma_index == 0:
        ...
        expected_value_types = [
            eval(x) for x in expected_value_type_str.split(",")
        ]
        if expected_value_types == [float, int] or expected_value_types == [
            int,
            float,
        ]:
            raise AssertionError(
                f"{expected_value_type_str} is an invalid type specification for flag={flag_name} as thetypes are interconvertible"
            )
    ...
    for evt_index, expected_value_type in enumerate(expected_value_types):
        ...
        # if expected type is bool, accept float/int values of 0 as False and 1 as True
            ...
            if flag_value == 1:
                ...
                flag_problems[evt_index] = (
                    f"Could not interpret numerical value {flag_value} as bool"
                )
            ...
            if flag_value is None:
                ...
                if flag_value.lower() == "none":
                    ...
                    flag_problems[evt_index] = (
                        f"Could not interpret string {flag_value} as None"
                    )
            else:
                flag_problems[evt_index] = (
                    f"Could not interpret value {flag_value} of type {type(flag_value)} as None"
                )
        ...
        # if expected type is a tuple and the flag value is a string
            ...
            try:
                ...
                flag_problems[evt_index] = (
                    f"Could not interpret string value {flag_value} as a tuple!"
                )

        # convert value int or float if required and possible
        elif expected_value_type in (float, int):
            try:
                flag_value = expected_value_type(
                    pd.to_numeric(flag_value, downcast="integer")
                )
            except (ValueError, TypeError):
                flag_problems[evt_index] = (
                    f"Could not interpret value={flag_value} as a {expected_value_type}!"
                )
        ...
        # failure to interpret <flag value> as <expected_value_type>
            ...
            flag_problem = f"flag {flag_name} was expected to be of type {expected_value_type_str}, got {type(flag_value)}"
        ...
        if not pd.isnull(flag_checks) and flag_problems[evt_index] is None:
            ...
            if not eval(flag_checks.format(flag="flag_value")):
                flag_problems[evt_index] = error_msg.format(
                    flag=flag_value, flag_name=flag_name
                )
    ...
    if all(x is not None for x in flag_problems):
        flag_problems_str = "\n- ".join(flag_problems)
        raise AssertionError(
            f"Could not set the value of the flag '{flag_name}' to '{flag_value}'. "
...
class FlagsManager:
    def __init__(self):
        ...
        self.compound_path_flags_with_defaults = [
            "STG_ProcessedDataPath",
            "STG_OdorAreaPath",
        ]
    ...
    def initialize_defaults(self, which=None):
        ...
        for flag_name, (
            ...
            is_not_a_compound_path_flag = (
                flag_name not in self.compound_path_flags
            )

            if (
                flag_name != "STG_MotherOfAllFolders"
                and is_not_a_compound_path_flag
                and not self.is_flag_deprecated(flag_name)
            ):
    ...
    def _expand_path_relative2moaf(self, flag_value):
        ...
        possible_path = (
            pl.Path(self["STG_MotherOfAllFolders"]) / flag_value
        ).resolve()
        ...
        for flag_name, flag_value in flags.items():
            ...
            if not self.is_flag_known(flag_name):
                logging.getLogger("VIEW").info(
                    "Ignoring update request for unknown flag",
            ...
            if flag_name in self.compound_path_flags:
                if flag_name in self.compound_path_flags_with_defaults:
                    self._check_update_compound_flag(
                        flag_name, flag_value, must_exist=False
                    )
                else:
                    self._check_update_compound_flag(
                        flag_name, flag_value, must_exist=True
                    )
    ...
    def is_flag_deprecated(self, flag_name):
        ...
        return (
            self.flags_def_df.loc[flag_name, "Flag Description"].lower()[:10]
            == "deprecated"
        )
    ...
    def _check_update_mother_of_all_folders(self, moaf):
        ...
        assert possible_moaf_path.is_dir(), ValueError(
            "Error updating the flag STG_MotherOfAllFolders! The value specified for it does not point to an "
    ...
    def _check_update_compound_flag(
        self, flag_name, flag_value, must_exist=True
    ):

        assert (
            flag_name in self.compound_path_flags
        ), f"{flag_name} is not one of {self.compound_path_flags}"
        ...
        try:
            ...
            # this happens when the parent of temp_path does not exist
            if must_exist:
                raise FileNotFoundError(
                    f'I am not able to figure out the path for "{flag_name}". Specifed value ({flag_value}) does not point to an existing folder, neither does {temp_path}. VIEW can unfortunately not work without this folder. Please check and if required create the folder.'
                ) from fnfe
    ...
    def initialize_compound_flags_with_defaults(self):
        ...
        for flag_name in self.compound_path_flags_with_defaults:
            if flag_name not in self.flags:
                flags2update[flag_name] = self.flags_def_df.loc[
                    flag_name, "Flag Default Value"
                ]
    ...
    def check_compound_flag_initialization_status(self):
        ...
        return {
            flag: (
                self.is_flag_state_default(flag)
    ...
    def get_subgroup_definition(self, subgroup):
        ...
        assert (
            subgroup in self.flags_def_df["Flag Subgroup"].values
        ), f"Invalid Subgroup {subgroup} specified"
        flags_def_df_reset = self.flags_def_df.reset_index()

        def selection_criteria(df):
            subgroup_mask = df["Flag Subgroup"] == subgroup
            non_deprecated_mask = df["Flag Description"].apply(
                lambda s: s[:10].lower() != "deprecated"
            )
    ...
    def get_flags_filtered_by_subgroup(self, subgroups: list):
        ...
        df = pd.DataFrame()
        for _index, row in self.flags_def_df.iterrows():
    ...
    def read_flags_from_yml(self, yml_filename):
        ...
        self.update_flags(
            {"STG_MotherOfAllFolders": str(pl.Path(yml_filename).parent)}
        )
    ...
    def write_flags_to_yml(self, yml_filename):
        ...
        for flag_name in self.compound_path_flags:
            if flag_name in self.flags:  # noqa: SIM102
                if not self.is_flag_state_default(flag_name):
                    path = pl.Path(self.flags[flag_name])
                    if path.is_absolute():
                        dict2write[flag_name] = os.path.relpath(
                            path, self["STG_MotherOfAllFolders"]
                        )

        dict2write["STG_MotherOfAllFolders"] = (
            "!!parent of this file will be used!!"
        )
    ...
    def get_animal_op_dir_path(self):
        ...
        """
        return (
            pl.Path(self.get_op_dir_str()) / f"{self.get_current_animal_id()}"
        )
    ...
    def get_move_corrected_data_path_for_animal(self, animal):
        ...
        return (
            self.get_processed_data_dir_path()
            / "MovementCorrectedData"
            / animal
        )

    def get_existing_filename_in_coor(self, extension, measurement_label=""):

        return check_get_file_existence_in_folder(
            folder=self.get_coor_dir_str(),
    ...
    def get_existing_area_filepath(self, measurement_label=""):
        ...
        filename = check_get_file_existence_in_folder(
            folder=self.get_area_dir_str(),
    ...
    def get_existing_lst_file(self, animal_name=None):
        ...
        possible_lst_extensions = get_format_specific_defs()[
            "extension"
        ].values
        return check_get_file_existence_in_folder(
            folder=self.get_list_dir_str(),
            stems=[self.get_current_animal_id()],
    ...
    def get_lst_file_stem(self):
        ...
        return str(
            pl.Path(self.get_list_dir_str()) / self.get_current_animal_id()
        )
    ...
    def get_gloDatamix_file_for_current_animal(self):
        ...
        return str(
            self.get_animal_op_dir_path()
            / f"{self.get_current_animal_id()}.gloDatamix.csv"
        )

    def get_pipeline_report_dir_for_animal(self, animal):

        return str(
            pl.Path(self.get_op_dir_str()) / "Pipeline Reports" / animal
        )
    ...
    def get_roi_filename_stem_for_current_animal(self):
        ...
        return self.get_roi_filename_stem_for_animal(
            animal=self.get_current_animal_id()
        )
    ...
    def is_flag_state_default(self, flag_name):
        ...
        assert (
            flag_name in self.flags_def_df.index
        ), f"Unknown flag name {flag_name}"
        ...
        if flag_value_is_inited:
            flag_value_is_default = self.flags[
                flag_name
            ] == check_correct_flag(
                flag_name=flag_name,
    ...
    def get_measurement_label(self, measurement_row):
        ...
        labels2use = [
            ...
            if x not in labels_to_exclude and x in measurement_row
        ]
    ...
    def get_ctv_method_file(self):
        ...
        if (
            pl.Path(ctv_method_file).is_absolute()
            and pl.Path(ctv_method_file).is_file()
        ):
            return ctv_method_file
        elif not self.is_flag_state_default("STG_MotherOfAllFolders"):
            possible_file = self._expand_path_relative2moaf(
                self["CTV_MethodFile"]
            )
        ...
        raise FileNotFoundError(
            f"Could not interpret the specified CTV Method File ({ctv_method_file}) "
            f"as a file"
        )
    ...
    def get_bleach_corr_patch_size(self):
        ...
        if type(flag_value) is int:
            ...
            if len(flag_value) == 2:
                ...
                raise ValueError(
                    f"Flag `LE_BleachPatchSize` expects an integer or a tuple of two integers. "
                    f"Got {flag_value}"
                )
...
def interpret_filter_params(
    filter_behaviour_switch, size_in_space, size_in_time
):
