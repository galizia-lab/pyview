...
class TapestryConfig:
    ...
    def iterrows(self):
        ...
        assert len(yml_keys) == len(
            set(yml_keys)
        ), f"In {self.yml_file}, Duplicate row names found! Please make sureeach row has a unique name."

        first_row = yml_contents_items[0][1]
        assert (
            "animal" in first_row
        ), f"First row of all tapestry config files must contain the entry 'animal'.This was not found in {self.yml_file}. Please add one!"
        assert (
            "measus" in first_row
        ), f"First row of all tapestry config files must contain the entry 'measus'.This was not found in {self.yml_file}. Please add one!"
        assert (
            len(first_row["measus"]) > 0
        ), f"Entry 'measus' in the first row of {self.yml_file} is empty, which is invalid. Please add some measurement to it!"

        # check if all entries have the correct type
        for _ind, (k, v) in enumerate(yml_contents_items):
            for entry, expected_type in optional_entry_def.items():
                if entry in v:
                    tpye = type(v[entry])
                    assert (
                        tpye is expected_type
                    ), f"Entry '{entry}' in {self.yml_file} is expected to be a {expected_type}. Got {tpye} instead"
            yield k, v
