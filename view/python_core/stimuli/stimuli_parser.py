...
class StimuliParamsParser:
    def __init__(self):

        self.param_def_df = pd.DataFrame()
        self.param_def_df["column_name"] = [
            "StimON",
            "StimOFF",
            "StimONms",
            "StimLen",
    ...
    def get_specified_unspecified_params(self, row: pd.Series):
        ...
        for column_name in self.param_def_df.index.values:
            if column_name in row:
                if self.metadata_def.is_value_default(
    ...
    def parse_row(self, row: pd.Series):
        ...
        specified_params, unspecified_params = (
            self.get_specified_unspecified_params(row)
        )
        ...
        if len(specified_params) == 0:
            ...
            commas_per_specified_param = {
                x: row[x].count(",") if type(row[x]) is str else 0
            ...
            assert all(
                x in (n_stim - 1, 0)
                for x in commas_per_specified_param.values()
            ...
            param_values = StimuliParams(
                columns=self.param_def_df.index,
                index=pd.RangeIndex(0, n_stim, 1),
                dtype=object,
            )
            ...
            for (
                ...
                if (n_commas == n_stim - 1) and (n_stim > 1):
                    ...
                    param_values.loc[:, specified_param] = (
                        self._parse_comma_separated_entry(
                            specified_param, row[specified_param]
                        )
                    )
            ...
            if n_stim == 1:
                second_param_values = {}
                for param_name, second_param_name in self.param_def_df[
                    "second_stim"
                ].items():
                    if not self.metadata_def.is_value_default(
                        second_param_name, row[second_param_name]
                    ):
                        second_param_values[param_name] = (
                            self._parse_comma_separated_entry(
                                param_name, row[second_param_name]
                            )
                        )
                    else:
                ...
                if any(
                    v is not None
                    for k, v in second_param_values.items()
                    if k.startswith("Stim")
                ):
                    param_values_new = StimuliParams(
                        columns=self.param_def_df.index,
                        index=pd.RangeIndex(0, 2, 1),
                        dtype=object,
                    )
                    ...
                    for (
                        param_name,
                        second_param_value,
                    ) in second_param_values.items():
                        param_values_new.loc[1, param_name] = (
                            second_param_value
                        )
