...
class BaseROIMarker(BlankROIMarker):
    ...
    def draw(self, frame):
        ...
        for roi_mask, col, _label in self.roi_mask_color_label_tuples:
...
class ROIMarkerSingleColor(BaseROIMarker):
    def __init__(
        ...
        perimeter_color,
        roi_data_dict: dict,
        unexcluded_frame_size,
        excluder: Excluder2D,
    ):
        super().__init__(roi_data_dict)
        for _roi_label, roi in roi_data_dict.items():
            perimeter_mask, boolean_mask = roi.get_perimeter_mask(
                unexcluded_frame_size
            )
            perimeter_mask_excluded = excluder.exclude_from_frame(
                perimeter_mask
            )
...
class ROIMarkerQualitativeColors(BaseROIMarker):
    def __init__(
        self, roi_data_dict: dict, unexcluded_frame_size, excluder: Excluder2D
    ):

        super().__init__(roi_data_dict)
        roi_labels_sorted = sorted(roi_data_dict.keys())
        colors = get_qualitative_colors(len(roi_labels_sorted))
        for roi_label, color in zip(roi_labels_sorted, colors, strict=True):
...
class ROIMarkerQualitativeColorsBlankBackground(ROIMarkerQualitativeColors):
    def __init__(
        ...
        super().__init__(
            ...
            unexcluded_frame_size=unexcluded_frame_size,
            excluder=excluder,
        )
...
def get_roi_marker_3D(
    flags, fg_color, unexcluded_frame_size, excluder, measurement_label
):
