...
from view.python_core.areas import get_area_for_p1
from view.python_core.flags import FlagsManager
from view.python_core.movies.colorizer import get_colorizer_2D
from view.python_core.movies.colorizer.aux_funcs import (
    stack_duplicate_frames,
)
from view.python_core.movies.data_limit import (
    get_data_limit_decider_2D,
)
from view.python_core.movies.data_to_01 import get_normalizer
from view.python_core.movies.excluder import Excluder2D
from view.python_core.movies.rotate import get_frame_rotator
from view.python_core.movies.spatial import get_spatial_processor
from view.python_core.movies.static_border import (
...
class OverviewColorizerAnnotator:
    def __init__(self, flags, p1):
        ...
        (
            self.colormap,
            self.bg_color_mpl_compliant,
            self.fg_color_mpl_compliant,
        ) = interpret_flag_SO_MV_colortable(
            SO_MV_colortable=flags["SO_MV_colortable"],
            fg_color=flags["SO_fgColor"],
            bg_color=flags["SO_bgColor"],
        )

        # initialize excluder
        self.excluder = Excluder2D(
            cutborder_x=flags["SO_cutborder"],
            cutborder_y=flags["SO_cutborder"],
        )
        ...
        self.spatial_processor = get_spatial_processor(
            filter_space_flag=flags["Signal_FilterSpaceFlag"],
            filter_space_size=flags["Signal_FilterSpaceSize"],
        )

        # initialize object for deciding data limit
        self.data_limit_decider = get_data_limit_decider_2D(
            flags=flags, frame_mask=area_mask_2D
        )
        ...
        self.colorizer = get_colorizer_2D(
            flags=flags,
            p1=p1,
            colormap=self.colormap,
            bg_color=self.bg_color_mpl_compliant,
        ...
        self.roi_marker = get_roi_marker_2D(
            flags=flags,
            measurement_label=p1.metadata.ex_name,
            fg_color=self.fg_color_mpl_compliant,
            bg_color=self.bg_color_mpl_compliant,
            unexcluded_frame_size=frame_size,
            excluder=self.excluder,
        )

        # initialize object for rotations
        self.frame_rotater = get_frame_rotator(
            rotate=flags["SO_rotateImage"], reverse=flags["SO_reverseIt"]
        )

        modified_frame_size = self.frame_rotater.transform_frame_size(
            revised_frame_size
        )

        # initialize object for adding colorbar
        self.colorbar_adder = get_static_border_adder_2D(
            flags=flags,
    ...
    def get_normalizer(self, data: np.ndarray):
        ...
        return get_normalizer(
            vmin=vmin, vmax=vmax, mv_individualScale=self.individual_scale
        )

    def colorize(self, data: np.ndarray, data_to_01_mapper):
        return self.colorizer.colorize(
            data=data, data_to_01_mapper=data_to_01_mapper
        )
    ...
    def add_colorbar(self, frame_data, static_frame):
        return self.colorbar_adder.composite(
            frame_data=frame_data, static_frame=static_frame
        )
...
def generate_overview_frame(flags, p1, feature_number=None):
    ...
    try:
        ...
        raise IndexError(
            f"The specified CTV has {n_features} features, so feature_number can be in [0, {n_features - 1}]. Some values of feature_number fall out of this range: {feature_number}"
        ) from inderr
...
def generate_overview_image(flags, p1):
    ...
    return colorize_overview_add_border_etc(
        overview_frame=overview_frame, flags=flags, p1=p1
    )
...
def colorize_overview_add_border_etc(
    ...
    if p1 is None:
        ...
        p1.area_mask = get_area_for_p1(
            frame_size=overview_frame.shape, flags=flags
        )
    ...
    data_to_01_mapper = overview_generator.get_normalizer(
        overview_frame_preprocessed
    )

    # get static frame
    try:
        static_frame = overview_generator.colorbar_adder.get_static_frame(
            data_to_01_mapper
        )
    ...
    # colorize the data
    overview_frame_colorized = overview_generator.colorize(
        data=overview_frame_preprocessed, data_to_01_mapper=data_to_01_mapper
    )

    # draw ROIs
    overview_frame_with_rois = overview_generator.add_rois(
        frame_data=overview_frame_colorized
    )

    # apply rotations
    overview_frame_rotated = overview_generator.rotate_frame(
        overview_frame_with_rois
    )

    # add colorbar
    overview_frame_final = overview_generator.add_colorbar(
        overview_frame_rotated, static_frame
    )
    np.set_printoptions(legacy="1.25")  # numpy now includes type info.
    logging.getLogger("VIEW").info(
        "SO_individualScale out of expected bounds",
        extra={
            "SO_individualScale": flags["SO_individualScale"],
            "min_max_bounds": data_to_01_mapper.get_data_limits(),
        },
    )
...
def pop_show_overview(
    flags, p1, label, stimulus_number=None, feature_number=None
):
    """
    ...
    for stim_ind in stimulus_number:
        ...
        overview_frames = generate_overview_frame(
            flags_copy, p1, feature_number=feature_number
        )
    ...
    for col_ind, stim_ind in enumerate(stimulus_number):
        for feature_ind, overview_frame_this_feature in enumerate(
            overview_frames_columns[col_ind]
        ):
            row_ind = feature_ind
            ...
            def format_coord(
                x_img_data,
                y_img_data,
                overview_frame_this_feature=overview_frame_this_feature,
                legendfactor=legendfactor,
            ):
                y_overview = int(y_img_data + 0.5)
                x_overview = int(x_img_data + 0.5)
                if (
                    0 <= y_overview < p1.metadata.format_y
                    and 0 <= x_overview < p1.metadata.format_x
                ):
                    z = (
                        overview_frame_this_feature[x_overview, y_overview]
                        * legendfactor
                    )

                    reportline = (
                        "Location: x=%.0i, y=%.0i, CTV Value (*%.0i)= %2.3f"
                    )
                    return reportline % (
                        x_img_data,
                        y_img_data,
