...
def add_MS_trace(data_in: np.ndarray, p1_metadata, flags, p1_extra_metadata):
    ...
    # ms data is:
    ms_data_file = (
        pl.Path(flags["STG_MotherOfAllFolders"])
        / p1_extra_metadata["filepath"]
    )  # this starts at 01_DATA
    if ms_data_file.exists():
        ...
        ms_resampled = np.interp(
            movie_time, ms_time, ms_data, left=np.nan, right=np.nan
        )
