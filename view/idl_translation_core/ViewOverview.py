...
):  # if Master_Hannah2018.py is importing this file
    import IDL
    import ImageALlocal
    import View
    import View_gr_reports
else:  # if we are importing this file from a package, e.g., using view.idl_translation_core.ViewOverview
    from view.idl_translation_core import (
        IDL,
        ImageALlocal,
        View,
        View_gr_reports,
...
def save_movie_file_xyt(dataMtrx, fps=24, bitrate="256k", movie_filename=""):
    ...
    def make_frame(t):
        ...
        ax.axis("off")
        ax.set_title(
            "Frame " + str(int(zlen / duration * t)) + "/" + str(zlen)
        )
        ax.imshow(
            dataMtrx_corrected[:, :, int(zlen / duration * t)],
            clim=(scaleMin, scaleMax),
        )
        # fig.colorbar()
...
def singleoverviews(flags_input, p1):
    ...
    localVIEW_batchmode = (
        flag.VIEW_batchmode
    )  # ? I keep it to translate literally, as fixed variable for now
    withinarea = (
        flag.SO_withinArea
    )  #  limit false-color output to within mask defined in .area file
    ...
    centerimage2zero = (
        individualscale // 10
    )  # all values up to 9 do not center scale image
    ...
    if (individualscale > 100) and (individualscale < 200):  # then begin
        indiscale3factor = (
            individualscale - 100
        ) / 100.0  # ; set to 0.2 (i.e. 120 for individualscale) for 20 % border to be ignored when scaling usind idividualscale eq 3
        individualscale = 3
    # endIF
    ...
    markcoordinates = (
        flag.RM_FotoOk
    )  # ]) ;if set, includes a black square at the glomeruli chosen
    ...
    #
    annotatefactor = (
        100  # ; factor to multiply minimum and maximum with when annotating
    )
    ...
    if unsharpmaskfilter:
        print(
            "*********  Using unsharp mask in SingleOverviews!! ***********************************************"
        )
        print(
            "not advised - I need to check the maths, because high frequency noise is deleterious"
        )
    #
    ...
    startframe = (
        flag.CTV_firstframe
    )  # in IDL, this was flag.CTV_firstframe only (without CTV)
    endframe = (
        flag.CTV_lastframe
    )  # in IDL, this was flag.flastframe only (without CTV)
    ...
    # endif
    #
    if individualscale == 0:
        scalestring = (
            " "
            + str(flag.SO_MV_scalemin)
            + "/"
            + str(flag.SO_MV_scalemax)
            + " "
        )
    elif individualscale == 1:
        scalestring = " indSc "
    elif individualscale == 2:
        scalestring = " " + str(minimum) + "/" + str(maximum) + " "
    elif individualscale == 3:
        scalestring = " SindSc "
    elif individualscale == 5:
        scalestring = " P5Sc "
    elif individualscale == 6:
        scalestring = " P6Sc "
    elif individualscale == 7:
        scalestring = " P7Sc "
    else:
        scalestring = (
            " P" + str(centerimage2zero) + str(individualscale) + "Sc "
        )

    if method == 2:
        borderstring = (
            ": start=" + str(startframe + 1) + ", end=" + str(endframe + 1)
        )
    else:
        borderstring = ": "
    #
    ...
    if exportTIFFframe:
        if localVIEW_batchmode:
            outfile = (
                flag.STG_OdorReportPath
            )  # this is not the file, but the folder
        else:
            outCanvasFile = IDL.dialog_pickfile(
                flag.STG_OdorReportPath,
                "TIF_" + p1.metadata.ex_name,
                write=True,
    ...
    if exportTIFFcanvas >= 1:  # THEN begin
        ...
        if localVIEW_batchmode:
            outCanvasFile = os.path.join(
                flag.STG_OdorReportPath, flag.STG_ReportTag
            )  # ] $
        else:
            outCanvasFile = IDL.dialog_pickfile(
                flag.STG_OdorReportPath, "", write=True, defaultextension="tif"
            )
    ...
    #    tvlct, r, g, b, /get
    # ENDIF
    if exportTIFFcanvas == 1:
        outCanvasFile = outCanvasFile + ".tif"
    elif exportTIFFcanvas == 2:
        outCanvasFile = outCanvasFile + "_3D.tif"
    elif exportTIFFcanvas == 3:
        outCanvasFile = outCanvasFile + "_3D.raw"
    elif exportTIFFcanvas == 4:
        outCanvasFile = outCanvasFile + ".tif"
    ...
    if exportTIFFcanvas in [1, 4]:
        ...
        if os.path.isfile(outCanvasFile):
            TIFFcanvas, (R, G, B) = IDL.read_tiff(
                outCanvasFile
            )  # use plt.imsave to save
            ...
            if newcolumn:
                ...
                dummy_row_size = max(
                    [xySizeCanvas[0], (p1.metadata.format_x + 2 * border)]
                )  # x values
                dummy_col_size = (
                    xySizeCanvas[1] + p1.metadata.format_y + border
                )
                ...
                TIFFcanvas = dummy
                flag_RM_nextposition = (
                    border,
                    xySizeCanvas[1],
                )  # tuple with rows,columns values
                print(
                    "ViewOverview.singleoverviews: Next frame (new column) to be placed at: ",
                    flag_RM_nextposition,
                )
                ...
                dummy_row_size = max(
                    [
                        xySizeCanvas[0],
                        flag_RM_nextposition[0]
                        + p1.metadata.format_x
                        + border,
                    ]
                )
                dummy_col_size = max(
                    [
                        xySizeCanvas[1],
                        flag_RM_nextposition[1]
                        + p1.metadata.format_y
                        + border,
                    ]
                )  # rows
                dummy = IDL.bytarr(dummy_row_size, dummy_col_size)
                ...
                TIFFcanvas = dummy
                print(
                    "ViewOverview.singleoverviews: Next frame (not new column) to be placed at: ",
                    flag_RM_nextposition,
                )
                print("...TIFFcanvas size is: ", TIFFcanvas.shape)
        #       ENDELSE ; newColumn
        else:  # new file#    ENDIF else BEGIN ;create new canvas, add frame of border on each side
            TIFFcanvas = IDL.bytarr(
                p1.metadata.format_x + (2 * border),
                p1.metadata.format_y + 2 * border,
            )
            flag_RM_nextposition = [
                border,
                border,
            ]  # ;for right positioning of new data, see below
    #    END
    # endIF
    #
    ...
    if exportTIFFcanvas in [1, 4]:
        ...
        window10 = PIL.Image.new(
            mode, windowSize
        )  # creates an image into object window10
    ...
    if unsharpmaskfilter:  # THEN begin
        print(
            "ViewOverview.singleoverviews: unsharpmaskfilter with fixed settings - check"
        )
        print(
            "does not work, because smooth creates values that are too small - needs a smooth or median on the original data"
        )
        overviewframe = 2 * overviewframe - IDL.smooth(
            overviewframe, unsharpmasksize
        )
        #           overviewFrame = 2*overviewframe - smooth(overviewframe, unsharpmasksize, /edge_truncate)
        if unsharpmasksize2 > 0:
            overviewframe = 2 * overviewframe - IDL.smooth(
                overviewframe, unsharpmasksize2
            )
    #    endIF
    #
    #
    if individualscale == 1:
        ...
        individualscale == 3
    ):  # then begin ;take min and max only from central region
        if percentilescale:
            minimum = np.percentile(
                overviewframe[
                    round(indiscale3factor * p1.metadata.format_x) : round(
                        (1 - indiscale3factor) * p1.metadata.format_x
                    ),
                    round(indiscale3factor * p1.metadata.format_y) : round(
                        (1 - indiscale3factor) * p1.metadata.format_y
                    ),
                ],
                100 * percentilevalue,
            )
    ...
    #    endif
        ...
        maskframe = np.roll(
            maskframe,
            (p1.shiftX / shrinkfactor, p1.shiftY / shrinkfactor),
            axis=(1, 0),
        )  # in numpy, vertical is first
        ...
        if percentilescale:  # THEN begin
            if individualscale == 5:
                minimum = np.percentile(
                    overviewframe[positions], 100 * percentilevalue
                )
            maximum = np.percentile(
                overviewframe[positions], 100 - 100 * percentilevalue
            )
    ...
    if (
        centerimage2zero == 2
    ):  # then begin ;center the image at 0, simmetrically
    ...
    if exportTIFFcanvas in [1, 4]:
        ...
        window10 = IDL.xyouts(
            flag_RM_nextposition[1] + p1.metadata.format_y,
        ...
        window10 = IDL.xyouts(
            flag_RM_nextposition[1]
            + p1.metadata.format_y,  # place in the image to the right
        ...
        print(
            "ViewOverview.singleoverviews line ",
            get_linenumber(),
            ": canvas annotated with min max ",
            minimum,
            maximum,
        )
    ...
    if markcoordinates in [1, 3, 4]:
        ...
        for index, row in liste.iterrows():
            ...
            if markcoordinates == 4:
                overviewframe[
                    leftborder + 1 : rightborder, topborder + 1 : bottomborder
                ] = row.num_glo
    #           endIF
    #          endIF
    #        endfor
    ...
    #    ;this 'maske' file needs to be a greyscale tiff, only values of 0 (black) are taken
        ...
        print(
            "ViewOverview.singleoverviews ",
            get_linenumber(),
            ": overlay external tiff file not yet implemented *******************  ",
        )
    ...
    #    endIF
        ...
        print(
            "ViewOverview.singleoverviews ",
            get_linenumber(),
            ": overlay markCoordinates 5 not implemented yet *******************  ",
        )
    ...
    if markborder:
        ...
        if flipsides > 0:  # THEN begin
            if (
                flipframe
            ):  # THEN begin ; right side, green color is about a third of the colour scale
    ...
    if unsharpmaskfilter:  # THEN begin
        ...
        overviewframe[
            0,
            int(p1.metadata.format_y / 3) : int(2 * p1.metadata.format_y / 3),
        ] = setminimum
        overviewframe[
            int(p1.metadata.format_x / 3) : int(2 * p1.metadata.format_x / 3),
            0,
        ] = setmaximum
        overviewframe[
            -1,
    ...
    if (centerimage2zero == 0) or (centerimage2zero == 2):  # THEN begin
        #       ;scale image to byte
        frame2 = IDL.bytscl(
            overviewframe, MIN=setminimum, MAX=setmaximum
        )  # , TOP=!d.table_size)
        print(
            "SingleOverviews  ",
            get_linenumber(),
            ", Image scaled to min, max: ",
            setminimum,
            setmaximum,
        )
    #    endIF
        ...
        frameup = IDL.bytscl(
            overviewframe, MIN=0, MAX=setmaximum
        )  # ,   TOP=!d.table_size)
        framedown = IDL.bytscl(
            overviewframe, MIN=setminimum, MAX=0
        )  #   TOP=!d.table_size)
        if setminimum >= 0:
            framedown[:] = 255
    ...
    #    endIF
        ...
        print(
            "Singleoverviews  ",
            get_linenumber(),
            ": centerimage2zero with unexpected value",
        )
    # else
    #    ;morphoThreshold
    ...
    if zoomfactor != 1:
        frame2 = IDL.rebin(
            frame2,
            (p1.metadata.format_x) * zoomfactor,
            p1.metadata.format_y * zoomfactor,
            sample=1,
        )
    ...
    if flipframe == 2:
        frame2 = np.flipud(frame2)  # rotate 7
    #
    if not localVIEW_batchmode:
        ...
        print(
            "ViewOverview.singleovervies line ",
            get_linenumber(),
            ": frame2 has values ranging from ",
            np.min(frame2),
            np.max(frame2),
        )
        plt.imshow(
            frame2.T, cmap=ct_IDLpalette, origin="bottom", vmin=0, vmax=255
        )
    ...
    if exportTIFFframe:
        tifffilename = os.path.join(
            outfile, ImageALlocal.localodortext(flag, p1) + ".tif"
        )
    ...
    if exportTIFFcanvas in [1, 4]:
        ...
        TIFFcanvas[
            flag_RM_nextposition[0] : flag_RM_nextposition[0]
            + p1.metadata.format_x,
            flag_RM_nextposition[1] : flag_RM_nextposition[1]
            + p1.metadata.format_y,
        ] = frame2
        flag_RM_nextposition = (
            flag_RM_nextposition[0] + p1.metadata.format_x + border,
        ...
        print(
            "Remember and write to flags: New Nextposition is ",
            flag_RM_nextposition,
        )
    ...
    #    IF (exportTIFFcanvas eq 2) THEN begin
        ...
        #       ;NextPosition(1) = NextPosition(1) + border + p1.format_y
        print(
            "not implemented yet: frame written to multilayered TIFF ",
            outCanvasFile,
        )
        # TODO
    ...
    #    ENDif ; exportTIFFcanvas
        ...
        print(
            "ViewOverview.Singleoverviews:  exportTIFFcanvas 3 not implemented yet"
        )
    ...
    if drawscalebar:
        ...
        scalebar = np.tile(
            np.arange(p1.metadata.format_x * zoomfactor), (31, 1)
        )  # create array with repeated elements, 31 pixels wide
        ...
        #    endIF
        scalebar = IDL.bytscl(
            scalebar, MIN=np.min(scalebar), MAX=np.max(scalebar)
        )
    ...
    if exportTIFFcanvas in [1, 4]:  # THEN begin
        ...
        longSide = max(
            newsizecanvas[0:1]
        )  # ;11.693 inches is 29.7 cm, 10,118 is 25.7 cm
        shortSide = min(
            newsizecanvas[0:1]
        )  # ;8.268 inches is 21 cm, 6.695 inches is 17 cm
        A4resolution = max([longSide / 10.118, shortSide / 6.695])
        # create palette
        ct_IDLpalette = IDL.createPalette(
            flag.SO_MV_colortable
        )  # flag.SO_MV_colortable)
        ...
        IDL.write_tiff(
            outCanvasFile, TIFFcanvas, R, G, B, A4resolution, A4resolution
        )
        if drawscalebar:
            IDL.write_tiff(
                outCanvasFile + "_scb.tif", scalebar, R, G, B
...
def overview(p1, flag, method):
    if method == 0:
        ...
        print(
            "ViewOverview.overview: method not yet considered in python (19.9.2018)"
        )
    #
...
def overview0ctv(p1, flag):
    ...
    """
    print(
        "ViewOverview.overview0ctv not implemented yet in python (19.9.2018)"
    )
    overviewframe = 0
    return overviewframe
...
def overview10ctv(p1, flag):
    ...
    if CTVmethod < 0:
        print(
            "ViewOverview.overview10ctv does not implement flag.CTV_Method < 0, i.e. Overview10CTVLocal"
        )
        return
    ...
    if CTVmethod > 1000:
        print(
            "ViewOverview.overview10ctv does not implement flag.CTV_Method > 0, i.e. CurveToValueMathias"
        )
        return
    ...
    if TIMEfilter:  # dummy = smooth(dummy, Signal_FilterTimeSize)
        print(
            "ViewOverview.overview10ctv timefilter implemented differently from IDL version"
        )
    ...
    if CTVmethod == 12:
        ...
        maskFactor[firstframe : lastframe + 1] = 1.0 / (
            lastframe + 1 - firstframe
        )
    #    		end
    ...
    # 	;this is similar, BUT NOT IDENTICAL to method 35 in CTV, because there the maximum is in each pixel position separatedly
        ...
        position = p1.metadata.stimulus_on + np.argmax(
            singleCurve[
                p1.metadata.stimulus_on : p1.metadata.stimulus_on
                + int(3 * p1.frequency)
            ]
        )
        ...
        print(
            "ViewOverview.Overview10CTV: Maximum frame in this measurement was ",
            position,
        )
        maskFactor[position - 1 : position + 2] = 1.0 / 3
        maskFactor[p1.metadata.stimulus_on - 3 : p1.metadata.stimulus_on] = (
            -1.0 / 3
        )
    # 		end;35
    ...
    # 		  end
        ...
        print(
            "ViewOverview.Overview10CTV: WARNING: wrong method in Overview10ctv, set flag[ctv_method]"
        )
    # ENDCASE
    ...
    for i in range(p1.metadata.frames):  # do begin
        ...
        overviewFrame = overviewFrame + dummy[:, :, i] * maskFactor[i]
    print(
        "ViewOverview/overview10ctv: created frame with maskFactor, sum is :",
        np.sum(maskFactor),
    )

    if SPACEfilter:  # dummy = smooth(dummy, Signal_FilterTimeSize)
        print(
            "ViewOverview.overview10ctv spacefilter: neg value for GAUSSIAN filter. Set filter to: ",
            flag.Signal_FilterSpaceSize,
        )
        # in IDL, the filter was applied to all three dimension - but a time filter should only work on time
        overviewFrame = View.SpaceFilter(
            overviewFrame, flag.Signal_FilterSpaceSize
        )
...
def CurveToValue(curve, flag, p1):
    ...
    if CTVmethod < 0:
        print(
            "ViewOverview.CurveToValue: values below 0 not implemented yet (local)"
        )
    ...
    # IF (CTVmethod gt 1000) THEN result=CurveToValueMathias(curve)
        ...
        print(
            "ViewOverview.CurveToValue: values above 1000 not implemented yet (Mathias)"
        )
    # ;poor old Giovanni only has values between 0 and 999 (1000 is lost in ...)
    # IF (CTVmethod ge 0) AND (CTVmethod le 1000) THEN begin
    # CASE CTVmethod OF $
    ...
    #    21  :      result = (TOTAL(curve(p1.stimulus_on+2:p1.stimulus_on+4)) - TOTAL(curve(p1.stimulus_on-3:p1.stimulus_on-1)))/3
        ...
        result = np.sum(
            curve[flag.CTV_lastframe - 1 : flag.CTV_lastframe + 2]
        ) - np.sum(
            curve[flag.CTV_firstframe - 1 : flag.CTV_firstframe + 2]
        )  #  lastframe - firstframe
    #    22  :      result = (TOTAL(curve(lastframe-1:lastframe+1)) - TOTAL(curve(firstframe-1:firstframe+1)))/3
    elif CTVmethod == 23:
        result = np.sum(
            curve[
                p1.metadata.stimulus_on
                + 2 : round(p1.metadata.stimulus_on + 3 * p1.frequency)
            ]
        ) - np.sum(
            curve[
                round(
                    p1.metadata.stimulus_on - 3 * p1.frequency
                ) : p1.metadata.stimulus_on
                - 1
            ]
        )  #  lastframe - firstframe
    ...
    #    35 :       result = MAX(curve(p1.stimulus_on:p1.stimulus_on+3*p1.frequency))- Mean(curve(p1.stimulus_on-2:p1.stimulus_on))  ;
        ...
        result = np.max(
            curve[p1.stimulus_on : round(p1.stimulus_on + 3 * p1.frequency)]
        ) - np.mean(
            curve[p1.stimulus_on - 2 : p1.stimulus_on - 1]
        )  #  lastframe - firstframe
...
def Depracated_ExportMovie(flag, p1):
    ...
    mv_xgap = (
        flag.mv_xgap
    )  # border of frame, total (i.e. half of this on each side)
    ...
    mv_realTime = (
        flag.mv_realTime
    )  # 24 # ,		NOT IMPLEMENTED	$  ; insert frames per second of the movie, 0 for no realTime, 24 for MPEG, 15 for GIF->QuickTime
    ...
    mv_rotateImage = (
        flag.mv_rotateImage
    )  # 0 #,        $ ; rotate only image,  ; 0 for no action, 2 for 180 degrees
    mv_cutborder = (
        flag.mv_cutborder
    )  # 1 #,	  		$ ; die Pixelgröße von Filtersize wird ringsherum vom Bild abgeschnitten
    mv_morphoThreshold = (
        flag.mv_morphoThreshold
    )  # False #,	$;;1; substitutes lower range with morphological image to be taken from file
    mv_withinMask = (
        flag.mv_withinMask
    )  # False #,#NOT IMPLEMENTED#			$;   = 0   ; limits output to within the mask in xxx.area
    ...
    mv_markStimulus = (
        flag.mv_markStimulus
    )  # True #,		$ ; marks stimulus application with a red box
    mv_percentileScale = (
        flag.mv_percentileScale
    )  # False #,	$; = 0 !!!!!!discontinued
    mv_indiScale3factor = (
        flag.mv_indiScale3factor
    )  # 0.2 # set to 0.2 for 20 % border to be ignored when scaling usind idividualscale eq 3
    mv_individualScale = (
        flag.mv_individualScale
    )  # 3 # 3 and 4 implemented;,	$; = fix(flag[so_indiScale])
    ...
    # endIF
    mv_percentileValue = (
        flag.mv_percentileValue
    )  # 0#NOT IMPLEMENTED# ,	$; = float(individualScale MOD 100)/100.0
    mv_correctStimulusOnset = (
        flag.mv_correctStimulusOnset
    )  #  0 #value to be added to stimulus onset
    mv_displayTime = (
        flag.mv_displayTime
    )  # False # #NOT IMPLEMENTED#time in ss:ms as figures
    mv_minimumBrightness = (
        flag.mv_minimumBrightness
    )  # 0 ##NOT IMPLEMENTED# 0.4 #; creates a mask that depends on the brightnes of the foto
    ...
    print(
        "In ViewOverview-ExportMovie: mv_individualScale not implemented yet, taking min/max of movie"
    )
    ...
    if mv_realTime > 1:
        ...
        print(
            "Calculated frame length in ExportMovie: ",
            time * 1000 / (noFrames),
        ...
        print(
            ...
            " frames for ",
            time,
            " seconds of film. Stimulus is between frames ",
            stimStart,
            stimEnd,
        )
    if mv_displayTime:
    ...
    if filterSpaceFlag:
        for i in np.arange(noFrames - 1):
            buffer[:, :, i] = sci.gaussian_filter(
                buffer[:, :, i], filterSpaceSize
            )
    ...
    if (
        mv_individualScale == 3
    ):  # scale to min/max of all frames, central region only
        pixMIN = np.min(
            buffer[
                int(p1_format_x * mv_indiScale3factor) : int(
                    p1_format_x * (1 - mv_indiScale3factor)
                ),
                int(p1_format_y * mv_indiScale3factor) : int(
                    p1_format_y * (1 - mv_indiScale3factor)
                ),
                :,
            ]
        )
    ...
    for i in np.arange(noFrames):
        ...
        frame = np.full(
            (frame_Xsize, frame_Ysize), pixMIN, dtype=float
        )  # borders are black
        ...
        if mv_cutborder > 0:
            frame[
                xgap2 + mv_cutborder : p1_format_x + xgap2 - mv_cutborder,
                ygap2 + mv_cutborder : p1_format_y + ygap2 - mv_cutborder,
            ] = buffer[
                mv_cutborder : p1_format_x - mv_cutborder,
                mv_cutborder : p1_format_y - mv_cutborder,
                i,
            ]
        else:
            frame[xgap2 : p1_format_x + xgap2, ygap2 : p1_format_y + ygap2] = (
                buffer[:, :, i]
            )
        ...
        if mv_markStimulus:
            ...
            if stimArray[i]:
                ...
                frame[
                    border : xgap2 - border,
                    ygap2 : ygap2 + xgap2 - border - border + 2,
                ] = (
                    pixMAX + valueStep
                )  # duftQuadrat rot
    ...
    save_movie_file_xyt(
        MovieArray3D, fps=24, bitrate="256k", movie_filename=outfilename
    )
...
def ExportMovie(flag, p1):
    ...
    mv_FirstFrame = np.min(
        (flag.FT_FirstFrame, p1.metadata.frames)
    )  # avoid first frames after last movie frame
    ...
    buffer_center = p1.sig1.copy()[
        :, :, mv_FirstFrame : mv_FirstFrame + noFrames
    ]
    ...
    if flag.Signal_FilterSpaceFlag:
        for i in np.arange(noFrames - 1):
            buffer_center[:, :, i] = sci.gaussian_filter(
                buffer_center[:, :, i], flag.Signal_FilterSpaceSize
            )
    ...
    if flag.mv_individualScale == 2:
        ...
        PIXmin = np.min(
            p1.sig1[:, :, mv_FirstFrame : mv_FirstFrame + noFrames]
        )
        PIXmax = np.max(
            p1.sig1[:, :, mv_FirstFrame : mv_FirstFrame + noFrames]
        )
    ...
    frame_Xsize = (
        p1.metadata.format_x
        + flag.mv_xgap
        + ((p1.metadata.format_x + flag.mv_xgap) % 2)
    )
    frame_Ysize = (
        p1.metadata.format_y
        + flag.mv_ygap
        + ((p1.metadata.format_y + flag.mv_ygap) % 2)
    )
    # the border is filled woth bottomValue in order to have a black border (with the appropriate color scale)
    buffer = np.full(
        (frame_Xsize, frame_Ysize, noFrames), bottomValue
    )  # no odd numbers for mpeg
    # dependent variables
    ...
    buffer[
        xgap2 : xgap2 + p1.metadata.format_x,
        ygap2 : ygap2 + p1.metadata.format_y,
        :,
    ] = buffer_center
    ...
    if (
        p1.stimulus_ISI > 0
    ):  # repetitive stimuli. Limited number not implemented yet
    ...
    # now use ffmpeg routine
    save_movie_file_xyt(
        buffer, fps=24, bitrate="1024k", movie_filename=outfilename
    )
