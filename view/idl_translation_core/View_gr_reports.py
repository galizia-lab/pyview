...
def gr_takefromlist(inputLabel, SelectValue, flag, selectformat="analyze"):
    ...
    InputOdorFile_xl = os.path.join(
        flag["STG_OdorInfoPath"], flag["STG_ReportTag"] + ".lst.xls"
    )
    InputOdorFile_lst = os.path.join(
        flag["STG_OdorInfoPath"], flag["STG_ReportTag"] + ".lst"
    )
    ...
    if selectformat == "analyze":
        ...
        raise NotImplementedError(
            "Currently, 'selectformat' can only be 'analyze' or 'subloop'"
        )

    if len(measus) == 0:
        print(
            "View_gr_reports.gr_takefromlist: requested measurements do not exist"
        )
        return (0, 0)
    ...
    for measu in measus:
        ...
        if (
            selectformat == "analyze"
        ):  # many rows taken from settings file, no info about columns
            if (
                countLoop == 0
            ):  # this is the first one to analyze, select new column
...
def subloop(p1, flag):
    # keep track
    print(
        "View_gr_reports.subloop: running measurement: ",
        p1.metadata.messungszahl,
        "***",
        p1.metadata.ex_name,
    )
...
def subloop_report(flag, p1):
    ...
    if reportflag == 11:
        ExportGlomeruli(flag, p1)  # , 1) # export entire time course
    ...
    if reportflag == 1200:
        ViewOverview.ExportMovie(
            flag.to_series(), p1
        )  # Report12mpeg #calles exportmovie, settings in imageALlocalsetexportmovieflags.pro!!!!
    if reportflag == 13:
        ReportTiff(flag.to_series(), p1)  # single multilayered file
    if reportflag == 14:
        ReportTiff(flag.to_series(), p1)  # single multilayered file, raw data
    if reportflag == 15:
        ReportTiff(flag.to_series(), p1)  # one file for each frame, TifF
    ...
    if reportflag == 20:
        ReportTiff(
            flag.to_series(), p1
        )  # single multilayered file, only first odour
    if reportflag == 21:
        ReportTiff(
            flag.to_series(), p1
        )  # single multilayered file, diff. layers ('odors') sideways
    ...
    if reportflag == 111:
        ExportGlomeruli(flag, p1)  # , 1) # same as 11
...
def read_glo_liste(
    flag, p1
):  # (NumListe, liste, inputFile, quiet=quiet, column4=column4):
    """
    ...
    inputfile = os.path.join(
        flag["STG_OdormaskPath"], flag["STG_ReportTag"] + ".coor"
    )
    if not os.path.isfile(
        inputfile
    ):  # file does not exist, test different layers. Not tested
        inputfile = os.path.join(
            flag["STG_OdormaskPath"],
            flag["STG_ReportTag"] + str(p1.metadata.viewlabel) + ".coor",
        )
    ...
    glom_liste = pd.read_csv(
        inputfile, sep="\t", encoding="latin-1", names=colnames, header=None
    )
    num_glo = glom_liste.y_glo[0]  # only read this many glomeruli
    glom_liste.drop(
        [0], inplace=True
    )  # drop the first line, it only contains num_glo
    glom_liste = glom_liste.iloc[
        :num_glo, :
    ]  # keep only the first num_glo glomeruli
    # convert to integers
...
def ExportGlomeruli(flags, p1, measurement_row):
    ...
    reportAreas = (
        flag.RM_ROITrace
    )  # if set, reads a tiff file and gives glodatamix of the areas in there, instead of a list of glomeruli/squares
    ...
    GlomeruliListFile = os.path.join(
        flag.STG_OdormaskPath, flag.STG_ReportTag + ".coor"
    )
    ...
    if (
        reportAreas == 0
    ):  # THEN begin ; get information about glomeruli coordinates
        #   readListe, GlomeruliNum, liste, GlomeruliListFile, column4=separateLayers
        ...
        if sum(
            gloList.duplicated(["num_glo"])
        ):  # there is at least one duplicate
            print(
                "ExportGlomeruli: Glomerulus liste badly defined. Double Glomerulus "
            )
            # break
    ...
    ###########;all reportareas ge 1 use areas and not coordinates
        ...
        # ;shift tiff file
        maskAreas = np.roll(
            maskAreas, (p1.metadata.shiftX, p1.metadata.shiftY), axis=(1, 0)
        ...
        for glo in range(253):  # do begin; exclude 255 =black, 0= white
            ...
            if (
                sum_glo > 1
            ):  # then begin ;*********** areas with only 1 pixel are NOT accepted because averaging crashes below
                # IF (GlomeruliNum eq 0) THEN gloList = [glo,count] ELSE gloList = [gloList, [glo,count]]
                # GlomeruliNum = GlomeruliNum + 1
                gloList.loc[glo] = [
                    glo,
                    sum_glo,
                ]  # this syntax would overwrite equal glo
    # endif
    # endFOR
    ...
    # endIF
        ...
        # maskAreas = shift(maskAreas, p1.metadata.shiftX, p1.metadata.shiftY)
        maskAreas = np.roll(
            maskframe, (p1.metadata.shiftX, p1.metadata.shiftY), axis=(1, 0)
        )  # in numpy, vertical is first
        # ;consider shrinkfactor
        # maskAreas = rebin(maskAreas, p1.metadata.format_x, p1.metadata.format_y, /sample)
        gloList = pd.DataFrame(columns=["glo", "count"])
        ...
        if reportAreas == 3:
            roi_filepath = flags.get_existing_filename_in_coor(
                measurement_label=p1.metadata.ex_name, extension=".roi"
            )
            ...
            roi_masks_list = [
                roi_masks_3D[:, :, ind] for ind in range(roi_masks_3D.shape[2])
            ]
    ...
    GlomeruliNum = len(
        gloList
    )  ##how many rows, corresponds to number of glomeruli
    ...
    Num_columns = [
        "NGloTag",  # 		;integer name of glomerulus, or fictive name >300, or incrementative >1000
    ...
    ArrayNumInfos = pd.DataFrame(
        np.zeros([GlomeruliNum, len(Num_columns)], dtype=int),
        columns=Num_columns,
    )
    ...
    # ;each position with the following meaning:
    Txt_columns = [
        "TGloInfo",  # 	;comment text for the glomerulus
        ...
        "T_dbb2",
    ]  # 	;second dbb file - can be misused (e.g. male/female)
    # TODO : here I initialize ArrayTxtInfos with numbers, but I need text - really, I only need glo lines
    ArrayTxtInfos = pd.DataFrame(
        np.zeros([GlomeruliNum, len(Txt_columns)]), columns=Txt_columns
    )
    # make ArrayGloTraces into a DataFrame with titles Frame0....FrameN
    Frame_columns = [f"Frame{s}" for s in range(p1.metadata.frames)]
    ArrayGloTraces = pd.DataFrame(
        np.zeros([GlomeruliNum, p1.metadata.frames]), columns=Frame_columns
    )
    ...
    ArrayTxtInfos.iloc[:, 5] = str(
        p1.metadata.pharma_time
    )  # stringzero(2,p1.metadata.pharma_time(0))+ ':' +stringzero(2,p1.metadata.pharma_time(1))+':'+stringzero(2,p1.metadata.pharma_time(2)) ; appplacation time of treatment
    ArrayTxtInfos.iloc[:, 6] = str(
        p1.metadata.os9time
    )  # stringzero(2,p1.metadata.os9time(0))    + ':' +stringzero(2,p1.metadata.os9time(1))    +':'+stringzero(2,p1.metadata.os9time(2)) ; appplacation time of treatment
    ...
    print(
        "ExportGlomeruli: os9time and pharma_time: ",
        p1.metadata.os9time,
        p1.metadata.pharma_time,
    )
    print(
        "ExportGlomeruli info: ",
        ArrayTxtInfos.iloc[0, 5],
        " ",
        ArrayTxtInfos.iloc[0, 6],
    )
    ...
    for glo, row in gloList.iterrows():
        ...
        if reportAreas in (
            1,
            2,
        ):  ##check that the right columns in gloList are used, the IDL code uses x and y
            ...
            ArrayTxtInfos.iloc[glo, 0] = (
                "Area" + str(row.num_glo) + "; Pixels: " + str(row.y_glo)
            )
        ...
        # bufferset = DUFT
            ...
            if reportAreas == 3:
                ArrayTxtInfos.iloc[glo, 0] = (
                    roi_datas[glo].write_to_text_line().rstrip("\n")
                )
            else:
                ArrayTxtInfos.iloc[glo, 0] = (
                    "ILTIS-AreaTIF "
            ...
            xborders = [
                x - radius,
                x + radius + 1,
            ]  # add 1, because python does not include the right value
            xborders = np.clip(
                xborders, 0, p1.metadata.format_x
            )  # simplified in python, no print feedback
            ...
            # ENDif
            yborders = np.clip(
                [y - radius, y + radius + 1], 0, p1.metadata.format_y
            )
            ...
            # bufferSet = DUFT
            # endELSE
            ArrayNumInfos.iloc[glo, 14] = (
                0  # there is only one odor in python, and no layers bufferSet ; layer/odor
            )
        ...
        #            positions = GloMask == 1
        # endELSE ;reportAreas
        ArrayGloTraces.iloc[glo, :] = ViewTools.calc_timetrace(
            p1.sig1, GloMask
        )  # x,y,t
        # python, moved here from below because CTV is always calculated
        ctv_handler_obj = PixelWiseCTVHandler(flags=flags, p1=p1)
        ArrayNumInfos.loc[glo, ctv_tag] = ctv_handler_obj.apply_pixel(
            ArrayGloTraces.iloc[glo, :]
        )
    # ;calculate Traces
    ...
    if exportOption:  # this is always the case,
        ...
        ArrayAll = pd.concat(
            [ArrayNumInfos, ArrayTxtInfos, ArrayGloTraces], axis=1
        )
