...
from view.idl_translation_core.bleach_correction import (
    fitlogdecay,
    get_bleach_weights,
)
...
def CalcSigMaster(flag, p1):
    ...
    if method1 == 0:
        p1 = CalcSigAll0(
            p1, flag
        )  # absolute values / 1000, effectively raw data
    ...
    #  END
        ...
        print("ViewCalcData.CalcDataMaster: flag.CalcMethod not implemented")
    return


def CalcSigAll3000(p1, flag):
    # settings values
    FlagRatioSetting = int(
        flag["LE_CalcMethod"] / 1000
    )  # first digit. int is like floor
    RatioSetting = FlagRatioSetting == 4  # is this a FURA measurement?
    startbackground = flag["LE_StartBackground"]  # background ab frame ..-
    prestim_endbackground = flag[
        "LE_PrestimEndBackground"
    ]  # background till n frames before stimulus
    ...
    if RatioSetting:  # fura measurement - so far we worked on raw1, i.e. 340
        ...
        p2, flag = CalcSigAll3000_raw(
            p2, flag
        )  # run the full program on raw2 (called raw1 here)
        # now calculate the ratio, put into p1.
        # 340/380 is p1/p2
        # but sig1 is deltaF/F, i.e. not F/F0, therefore add 1
        p1.sig1 = (p1.sig1 + 1) / (
            p2.sig1 + 1
        )  # times multiplyFactor for percentages, not defined in python
        # p1.raw1 = p2.raw1 #CalcSigAll3000 modifies raw1 also
        p1.metadata.bleachpar2 = (
            p2.metadata.bleachpar
        )  # keep track of the bleach parameters
        ...
        for i in range(
            p1.metadata.frames
        ):  # remove the effect of calculating F/F0
            p1.sig1[:, :, i] = p1.sig1[:, :, i] * backBackFrame
        print(
            "ViewCalcData/CalcSigAll3000: calculated ratio based on method 4xyy ",
            flag["LE_CalcMethod"],
        )
    ...
    if subtractBeginning:
        ...
        backbackframe = np.mean(
            p1.sig1[:, :, startbackground:endbackground], axis=2
...
def CalcSigAll3000_raw(p1, flag):
    ...
    lightmeanvalue = (
        1000.0  # ; raw data are first scaled to this mean light value
    )
    ...
    rawdatadelete = (
        flag["VIEW_DeleteRawData"] != 0
    )  # ; deletes raw data at the end, saves disk space
    logexcludeseconds = flag[
        "LELog_ExcludeSeconds"
    ]  # ]) ; how many seconds after stimulus onset to exclude, when set
    bleachstartframe = flag[
        "LE_BleachStartFrame"
    ]  # ] ); set to 2 to exclude 2 frames at the beginning from log-fit
    # frames before bleachstartframe are ignored for bleach correction
    loginitialfactor = flag[
        "LELog_InitialFactor"
    ]  # ] ) ; weight factor for curve stretch before stimulus onset
    ...
    myMethod = (
        int(flag["LE_CalcMethod"] / 100) % 10
    )  # second digit. contains settings for bleach correction
    ...
    startbackground = max(
        flag["LE_StartBackground"], 0
    )  # background ab frame ..- at least 0
    #    subtractbeginning 	= (startbackground >= 0)
    prestim_endbackground = flag[
        "LE_PrestimEndBackground"
    ]  # background till n frames before stimulus
    ...
    smoothvalue = int(
        flag["LE_CalcMethod"] % 100
    )  # last two digits, radius of filter in um
    unsharpmask = smoothvalue > 0

    if airbleach:
        print(
            "ViewCalcData.CalcSigAll3000: airBleach not implemented yet in Python"
        )
    ...
    if alperimeter:
        ...
        if shrinkfactor not in [0, 1]:  # both 0 and 1 mean 'no shrink'
            ...
            shrink = (
                1 / shrinkfactor
            )  # flag of 2 means half the size in each dimension
        ...
        if maskframe.shape != (
            ...
            print(
                "ViewCalcData.Calcsigall3000: AREA file has wrong size - maybe make with binning unequal 1?"
            )
            return  # jump out of the program
    else:  # there is no alPerimeter, i.e. no .Area file
        # 	;select central region of AL for correction
        maskframe = np.zeros(
            (p1.metadata.format_x, p1.metadata.format_y), dtype=int
        )
        maskframe[
            round(p1.metadata.format_x * cutedge) : round(
                p1.metadata.format_x * (1 - cutedge)
            ),
            round(p1.metadata.format_y * cutedge) : round(
                p1.metadata.format_y * (1 - cutedge)
            ),
        ] = 1
        # 	maskFrame[p1.format_x*CutEdge:p1.format_x*(1-CutEdge),p1.format_y*CutEdge:p1.format_y*(1-CutEdge)]=1
        # 	;now AL perimeter is in variable maskframe
        print(
            "CalcSigAll3000 - reference area for F and for Bleach is centre with edges cut at ",
            cutedge,
        )
    ...
    p1.metadata.backframe = np.mean(
        p1.raw1[:, :, startbackground:endbackground], axis=2
    )
    print(
        "ViewCalcData/CalcSigAll3000_raw: background start/stop is frame: ",
        startbackground,
        endbackground,
    )
    # takes the average for selected time frames
    ...
    if backvalue == 0:
        backvalue = 1  # safety net to avoid dividing by 0
    ...
    if unsharpmask:  # light scatter correction
        ...
        print(
            "Scattered light correction with ",
            smoothvalue,
            " pixels for ",
            p1.metadata.pixelsizex * smoothvalue,
            " microns. Factor:",
            smoothfactor,
        )
        print(
            "CalcSigAll3000: a negative factor means scattered light correction, a positive factor a spatial high-pass filter"
        )

        for i in range(p1.sig1.shape[2]):  # go through frames
            ...
            if smoothfactor > 0:
                ...
                oneframe = oneframe + smoothfactor * (
                    oneframe
                    - sci.gaussian_filter(
                        oneframe, smoothvalue, mode="nearest"
                    )
                )
            # 			endIF
            if smoothfactor < 0:
                ...
                oneframe = oneframe - smoothfactor * sci.gaussian_filter(
                    (
                        oneframe
                        - sci.gaussian_filter(
                            oneframe, smoothvalue, mode="nearest"
    ...
    if rawdatadelete:
        ...
        p1.raw1 = (
            p1.sig1.copy()
        )  # raw now contains scatter corrected "raw" data
    ...
    if not hasattr(p1, "foto1") or np.all(p1.foto1 == 0):
        p1.foto1 = np.mean(
            p1.sig1[:, :, startbackground:endbackground], axis=2
        )
    ...
    if nobleach:
        ...
        weights = get_bleach_weights(
            flags=flag,
            p1_metadata=p1_metadata,
            exclude_stimulus=excludestimulus,
        )
        ############
        print(weights)
        print(
            prestim_endbackground,
            logexcludeseconds,
            p1.metadata.frequency,
            stimulus_on,
            loginitialfactor,
            bleachstartframe,
        )
        if airbleach:
            print(
                "CalcSigAll3000: ********* AirBleach not implemented in Python yet"
            )
        ...
        # 				print, 'LogBleach done on reference measurement!'
            ...
            detract, opt_parms = fitlogdecay(
                correctarray,
                weights,
                not flag["VIEW_batchmode"],
                measurement_label,
            )
        ...
        if addnoise:
            ...
            detract = (
                ...
                + correctarray
                - sci.gaussian_filter(
                    correctarray, p1.metadata.frequency * noiseseconds
                )
            )
        ...
        if returnrawdata:  # setting is 5000
            for i in range(p1.metadata.frames):  # do begin
                ...
                p1.raw1[:, :, i] = (
                    p1.sig1[:, :, i] + 1
                ) * p1.metadata.backframe  # /multiplyfactor# ;raw data
    ...
    return (
        p1,
        flag,
    )  #  no return, p1 is modified already. Try return instead now (8/19)
...
def calc_deltaF(imgData, refRange):
    ...
    if np.any(dead_mask):
        ...
        logging.getLogger("VIEW").warning(
            "F0 for some pixels were 0, not normalizing these pixels!"
        )
...
def CalcSigAll0(p1, flag):
    ...
    print(
        "ViewCalcData/CalcSigAll0: Signals calculated (method 0). sig1 = raw1/1000"
    )
    # end ; of program
    return p1
...
def CalcSigAll1(p1, flag):
    ...
    print(
        "ViewCalcData/CalcSigAll1: Signals calculated (method 0). sig1 = raw2/1000"
    )
    # end ; of program
    return p1
...
def CalcSigAll4(p1, flag):
    ...
    print(
        "ViewCalcData/CalcSigAll4: Signals calculated (method 4). Fura, no thrills"
    )
    return p1
